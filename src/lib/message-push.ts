...
function wingMatchesChannel(wingName: string | null | undefined, channel: string): boolean {
    ...
    if (channel === 'technical')
        return /technical/i.test(w);
    if (channel === 'marketing')
        return /marketing|digital/i.test(w);
    if (channel === 'core')
        return /product|core/i.test(w);
    return false;
}
export async function resolveMessageRecipients(channel: string, senderId: string, peerId?: string | null): Promise<{
    employeeIds: string[];
    all: boolean;
    peerId: string | null;
}> {
    ...
    if (channel.startsWith('dm:')) {
        ...
        if (other)
            return { employeeIds: [other], all: false, peerId: other };
    ...
    if (channel === 'marketing' || channel === 'technical' || channel === 'core') {
        ...
        for (const r of accessRows) {
            if (r.employeeId && r.employeeId !== sender)
                ids.add(r.employeeId);
        }
        for (const e of employees) {
            if (e.id !== sender && wingMatchesChannel(e.wingName, channel))
                ids.add(e.id);
        }
