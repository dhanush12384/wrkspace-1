...
import {
	CHECKIN_REMINDER_OFFSETS,
	canCheckInNow,
	checkoutDue,
	getShiftPolicy,
	nowMinutesIST,
	todayKeyIST,
} from '@/lib/shift-policy';

function isOpenSession(row: { checkIn?: string | null; checkOut?: string | null; status?: string | null }) {
...
export async function assertCanCheckIn(emp: {
	...
	if (!gate.ok) {
		const err = new Error(
			`Unable to check in. Your check-in time was ${gate.checkInBy}. Request permission from admin.`,
		) as Error & { status?: number; code?: string; payload?: Record<string, unknown> };
		err.status = 403;
...
export async function processShiftCheckoutJobs(opts?: { notify?: boolean }) {
...
export async function maybeArriveHomeFromLocation(
	tripId: string,
	employeeId: string,
	lat: number,
	lng: number,
	...
	if (emp?.homeLat == null || emp?.homeLng == null) return null;
