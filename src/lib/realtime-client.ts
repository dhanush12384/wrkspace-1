...
export function connectRealtime(opts: {
    ...
    (async () => {
        try {
            ...
            socket.on('late_permission', (p: Record<string, unknown>) => opts.onAttendance?.({ ...p, action: p.action || 'late-permission' }));
            socket.on('shift:reminder', (p: Record<string, unknown>) => opts.onAttendance?.({ ...p, action: p.action || 'checkin_reminder' }));
            socket.on('payout:update', (p: Record<string, unknown>) => opts.onAttendance?.({ ...p, action: p.action || 'payout-update' }));
