...
export type SkillCategoryKey = 'programmingLanguages' | 'frontend' | 'backend' | 'database' | 'cloud' | 'devops' | 'tools' | 'softSkills';
...
export function profileFromEmployee(emp: Record<string, unknown> | null | undefined): ProfessionalProfile {
    ...
    const projects = parseJsonArray<ProfileProject & {
        url?: string;
        year?: string;
    }>(e.projects).map((p) => ({
        id: p.id || newId(),
    ...
    const certifications = parseJsonArray<ProfileCertification & {
        year?: string;
...
export function sanitizeProfessionalProfile(input: Partial<ProfessionalProfile>, opts?: {
    ...
    const experience = (input.experience || [])
        ...
        fileUrl: clip(x.fileUrl, 900000) || undefined,
    }))
    ...
    const education = (input.education || [])
        ...
        fileUrl: clip(q.fileUrl, 900000) || undefined,
    }))
        .filter((q) => q.institution || q.degree);
    ...
    const projects = (input.projects || [])
        ...
        fileUrl: clip(p.fileUrl, 900000) || undefined,
    }))
        .filter((p) => p.name);
    const certifications = (input.certifications || [])
        ...
        fileUrl: clip(c.fileUrl, 900000) || undefined,
    }))
        .filter((c) => c.name);
    const achievements = (input.achievements || [])
        ...
        fileUrl: clip(a.fileUrl, 900000) || undefined,
    }))
        .filter((a) => a.title);
    ...
    const out: Record<string, unknown> = {
        ...
        personalFileUrl: clip(input.personalFileUrl, 900000) || null,
        ...
        summaryFileUrl: clip(input.summaryFileUrl, 900000) || null,
        skillsFileUrl: clip(input.skillsFileUrl, 900000) || null,
