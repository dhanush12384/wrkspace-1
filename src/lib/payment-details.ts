/** Payment complete: UPI alone, or holder + account + bank + IFSC. */
...
export function parseEmployeePaymentBody(
	...
	const bankAccountNumber =
		body?.bankAccountNumber != null ? String(body.bankAccountNumber).trim().slice(0, 40) : '';
	const bankAccountNumberConfirm =
		body?.bankAccountNumberConfirm != null
	...
	const bankIfsc =
		body?.bankIfsc != null ? String(body.bankIfsc).trim().toUpperCase().slice(0, 20) : '';
