...
export interface AugustAttendanceStats {
    ...
    records: Array<{
        ...
        checkIn: string;
        checkOut: string;
        status: 'Present' | 'Absent' | 'Checked In' | 'Late' | 'Leave' | string;
    }>;
}

export interface EmployeeAugustReportData {
    employee: {
        id: string;
        firstName: string;
    ...
    stats: AugustAttendanceStats;
}

/**
 * Get distinct active working dates in August 2026 across the company or up to current date.
 */
export async function getAugustWorkingDates(): Promise<string[]> {
    const dates = await db.attendance.findMany({
        where: {
            date: {
                startsWith: '2026-08',
            },
    ...
    // Fallback: Generate dates from 2026-08-01 up to current date in August 2026
    const result: string[] = [];
    const now = new Date();
    const currentDay = now.getFullYear() === 2026 && now.getMonth() === 7 ? now.getDate() : 21;
    for (let day = 1; day <= currentDay; day++) {
        const dd = String(day).padStart(2, '0');
        result.push(`2026-08-${dd}`);
    }
    return result;
}

/**
 * Calculate August attendance statistics for a given employee.
 */
export async function getEmployeeAugustAttendanceData(
    employeeId: string,
    allWorkingDates?: string[]
): Promise<EmployeeAugustReportData | null> {
    ...
    const workingDates = allWorkingDates || (await getAugustWorkingDates());

    const employeeLogs = await db.attendance.findMany({
        where: {
            employeeId,
            date: {
                startsWith: '2026-08',
    ...
    const logMap = new Map<string, (typeof employeeLogs)[0]>();
    for (const log of employeeLogs) {
        logMap.set(log.date, log);
    }

    const records: AugustAttendanceStats['records'] = [];
    let presentCount = 0;
...
/**
 * Generate official August Attendance Statement PDF for an employee.
 */
export async function generateAugustAttendancePdf(
    data: EmployeeAugustReportData
): Promise<Uint8Array> {
    ...
    // 3. AUGUST SUMMARY KPI CARDS (4 boxes)
    const cardGap = 8;
    ...
    // 5. AUGUST DAY-BY-DAY ATTENDANCE LOG TABLE
...
/**
 * Generate HTML email template for August Attendance Statement.
 */
export function generateAugustAttendanceEmailHtml(data: EmployeeAugustReportData): {
    subject: string;
    text: string;
    html: string;
} {
...
export interface SendAugustAttendanceOptions {
    employeeId?: string;
    dryRun?: boolean;
}

export interface SendAugustAttendanceResult {
...
/**
 * Dispatch August Attendance emails with attached PDF statements.
 */
export async function sendAugustAttendanceReports(
    options: SendAugustAttendanceOptions = {}
): Promise<SendAugustAttendanceResult> {
    const { employeeId, dryRun = false } = options;
    ...
    const result: SendAugustAttendanceResult = {
    ...
    for (const emp of employees) {
        ...
        try {
            const reportData = await getEmployeeAugustAttendanceData(emp.id, workingDates);
            if (!reportData) {
                result.failed++;
                result.logs.push({
                    employeeId: emp.id,
                    ...
                    isBelowSixty: true,
                    status: 'FAILED',
                    error: 'Could not load employee attendance data',
                });
                continue;
            }

            if (reportData.stats.isBelowSixty) {
                result.belowSixtyCount++;
            } else {
                result.compliantCount++;
            }

            // Generate PDF
            const pdfBytes = await generateAugustAttendancePdf(reportData);
            const pdfBuffer = Buffer.from(pdfBytes);
            const { subject, text, html } = generateAugustAttendanceEmailHtml(reportData);
            ...
            if (dryRun) {
                ...
                result.logs.push({
                    ...
                    name: fullName,
                    email: emp.email,
                    attendancePercentage: reportData.stats.attendancePercentage,
                    isBelowSixty: reportData.stats.isBelowSixty,
                    status: 'SIMULATED',
                });
            } else {
                // Send via Resend
            ...
            console.error(`Failed to send August attendance report for ${emp.id}:`, err);
            result.failed++;
            result.logs.push({
                employeeId: emp.id,
                name: fullName,
