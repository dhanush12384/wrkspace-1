...
export function signEmployeeToken(employee: {
    ...
    return jwt.sign({ sub: employee.id, email: employee.email, role: employee.role, kind: 'employee' } satisfies EmployeeJwt, secret(), { expiresIn: '30d' });
}
export function signAdminToken(admin: {
    ...
    } satisfies AdminJwt, secret(), { expiresIn: '30d' });
}
export function signVerificationToken(user: {
    ...
    return jwt.sign({
        ...
        source: user.source,
    } satisfies VerificationJwt, secret(), { expiresIn: '12h' });
}
...
export function requireVerification(req: NextRequest): VerificationJwt {
    ...
    if (payload.kind !== 'verification' || !payload.sub)
        throw new Error('Unauthorized');
    return payload;
}
