...
export function buildEmployeeInsights(input: {
    ...
    const openSessions = att.filter((a) => a.checkIn && (!a.checkOut || String(a.checkOut).trim() === ''));
    ...
    const completed = tasks.filter((t) => /complete|done|closed/i.test(String(t.status || '')));
    const pending = tasks.filter((t) => /pending|progress|open/i.test(String(t.status || '')));
    const overdue = tasks.filter((t) => {
        if (!t.deadline)
            return false;
        if (/complete|done|closed/i.test(String(t.status || '')))
            return false;
    ...
    if (!strengths.length)
        strengths.push('Insufficient dense history yet — review raw logs below.');
    if (!weaknesses.length)
        weaknesses.push('No major automated risk signals from current logs.');
