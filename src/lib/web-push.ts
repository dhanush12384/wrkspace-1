...
export async function registerWebPush(_employeeId?: string) {
    ...
    try {
        ...
        if (isIosSafari() && !isStandalonePwa()) {
            console.info('[web-push] iOS Safari tab cannot receive push — install Add to Home Screen (PWA) first');
        ...
        if ('serviceWorker' in navigator) {
            ...
            await Promise.all(regs.map(async (reg) => {
                ...
                const isPlaceholder = script.includes('/firebase-messaging-sw.js') &&
                    !script.includes('/api/firebase-messaging-sw');
                const isRootMessaging = reg.scope === `${window.location.origin}/` && script.includes('firebase-messaging');
        ...
        if (!vapid) {
            console.warn('[web-push] NEXT_PUBLIC_FIREBASE_VAPID_KEY is not set — getToken will likely fail');
