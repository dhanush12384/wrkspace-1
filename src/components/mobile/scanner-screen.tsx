...
export function MobileScannerScreen({ onClose }: Props) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                await scanner.start({ facingMode: 'environment' }, { fps: 8, qrbox: { width: 240, height: 240 } }, (decoded) => {
    ...
    async function handleToken(raw: string) {
        ...
        try {
            ...
            setSubtitle(`${result.officeName || 'Office'} · ${result.distanceMeters ?? '?'}m away`);
            ...
            setSubtitle(api?.canRequestPermission
    ...
    return (<div className="fixed inset-0 z-[80] flex flex-col bg-[#0F172A] text-white">
			<div className="flex items-center gap-3 px-3 pb-2" style={{ paddingTop: 'max(12px, env(safe-area-inset-top, 0px))' }}>
				<button type="button" onClick={() => onClose(false)} className="flex size-10 items-center justify-center rounded-full bg-white/10" aria-label="Close scanner">
			...
			<div className="relative mx-4 min-h-0 flex-1 overflow-hidden rounded-2xl bg-black">
				...
				{!showCamera ? (<div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center">
						{phase === 'success' ? (<CheckCircle2 className="size-16 text-[#067647]"/>) : phase !== 'error' ? (<Loader2 className="size-12 animate-spin text-[#2B6BFF]"/>) : null}
						<p className="text-xl font-bold">{title}</p>
						<p className="text-sm text-white/75">{subtitle}</p>
						{detail ? (<p className="mt-2 rounded-xl bg-white/10 px-3 py-2 text-left text-sm text-[#FECACA]">
								{detail}
							</p>) : null}
						{permissionNote ? (<p className="text-sm text-sky-300">{permissionNote}</p>) : null}
						{phase === 'error' && canRequestPermission ? (<button type="button" disabled={requestingPermission} onClick={() => void requestLatePermission()} className="mt-2 rounded-xl bg-[#2563EB] px-6 py-3 text-sm font-semibold disabled:opacity-60">
								{requestingPermission ? 'Sending…' : 'Request permission'}
							</button>) : null}
						{phase === 'error' ? (<button type="button" onClick={() => void retry()} className="mt-2 rounded-xl bg-[#0047FF] px-6 py-3 text-sm font-semibold">
								Try again
							</button>) : null}
					</div>) : null}
				{showCamera ? (<div className="pointer-events-none absolute inset-0 flex items-center justify-center">
			...
			<div className="space-y-3 px-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-4">
				...
				{detail && phase === 'idle' ? (<p className="text-center text-xs text-[#FECACA]">{detail}</p>) : null}
				<form className="flex gap-2" onSubmit={(e) => {
            e.preventDefault();
            void handleToken(manual);
        }}>
					<input value={manual} onChange={(e) => setManual(e.target.value)} placeholder="Or paste office QR token" className="min-w-0 flex-1 rounded-xl border border-white/20 bg-white/10 px-3 py-3 text-sm text-white placeholder:text-white/40 outline-none focus:border-[#2B6BFF]"/>
					<button type="submit" className="rounded-xl bg-[#FF6B5A] px-4 text-sm font-bold text-white">
						Go
					</button>
				</form>
			</div>
