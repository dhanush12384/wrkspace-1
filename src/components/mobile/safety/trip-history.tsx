...
export function MobileTripHistory({ employee }: {
    employee: any;
}) {
    ...
    const center = useMemo(() => {
        if (polyline.length)
            return { lat: polyline[0].lat, lng: polyline[0].lng };
        if (employee?.homeLat != null)
            return { lat: Number(employee.homeLat), lng: Number(employee.homeLng) };
    ...
    return (<div className="flex h-full min-h-0 flex-col">
			...
			<div className="min-h-0 flex-1 overflow-y-auto p-4 pb-8">
				...
				{loading ? (<p className="py-8 text-center text-sm text-[#64748B]">Loading trips…</p>) : trips.length === 0 ? (<p className="py-8 text-center text-sm text-[#64748B]">No going-home trips yet</p>) : (<ul className="space-y-2">
						{trips.map((t) => {
                const active = t.id === selectedId;
                return (<button key={t.id} type="button" onClick={() => setSelectedId(t.id)} className={`w-full rounded-xl border px-3 py-3 text-left ${active
