...
export function MobileEmergencySos({ employee }: Props) {
    ...
    return (<div className="space-y-4 p-4 pb-8">
			<button type="button" disabled={busy} onClick={() => void triggerSos()} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#B42318] py-[14px] text-[15px] font-bold text-white disabled:opacity-60">
				{busy ? 'Sending…' : 'SOS — share my live location'}
			</button>
			<button type="button" onClick={openHomeRoute} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#E2E8F0] bg-white py-3 text-sm font-semibold text-[#0F172A]">
			...
			<div>
				...
				<div className="overflow-hidden rounded-[14px] border border-[#E2E8F0] bg-white">
					{EMERGENCY_NUMBERS.map((n, i) => (<a key={n.phone + n.name} href={`tel:${n.phone}`} className={`flex items-center justify-between px-4 py-3.5 ${i < EMERGENCY_NUMBERS.length - 1 ? 'border-b border-[#E2E8F0]' : ''}`}>
							<span>
