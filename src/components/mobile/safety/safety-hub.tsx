...
export function MobileSafetyHub({ employee, onOpen }: Props) {
    ...
    return (<div className="space-y-4 p-4 pb-8">
			{female ? (<div className="flex flex-col items-center rounded-[14px] border border-[#E2E8F0] bg-white p-5 text-center">
				...
				</div>) : (<div className="rounded-[14px] border border-[#E2E8F0] bg-white p-4">
			...
			<div className="rounded-[14px] border border-[#E2E8F0] bg-white p-4">
				...
				{incidents.length === 0 ? (<p className="mt-3 text-sm text-[#64748B]">No active SOS incidents.</p>) : (<ul className="mt-3 space-y-2">
						{incidents.map((inc) => (<li key={inc.id} className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm">
								<p className="font-bold text-rose-900">
									{inc.employee?.firstName} {inc.employee?.lastName} needs help
								</p>
								{inc.employee?.phone ? (<p className="mt-1 font-semibold text-[#0F172A]">
										Phone:{' '}
								...
								<a href={`https://www.google.com/maps?q=${inc.lat},${inc.lng}`} target="_blank" rel="noreferrer" className="mt-2 inline-flex rounded-lg bg-[#B42318] px-3 py-2 text-xs font-bold text-white">
									Open live map
								</a>
							</li>))}
					</ul>)}
			...
			{female ? (<>
					...
					{canEditHome ? (<button type="button" onClick={() => onOpen('home_pin')} className="w-full rounded-xl border border-amber-300 bg-amber-50 p-3.5 text-left">
                ...
                : 'Not set — use GPS or Maps link'}
					...
					<div className="overflow-hidden rounded-[14px] border border-[#E2E8F0] bg-white">
						{links.map((l, i) => {
                const Icon = l.icon;
                return (<button key={l.key} type="button" onClick={() => onOpen(l.key)} className={`flex w-full items-center gap-3 px-3 py-3.5 text-left ${i < links.length - 1 ? 'border-b border-[#E2E8F0]' : ''}`}>
									<span className="flex size-9 items-center justify-center rounded-[10px]" style={{ backgroundColor: `${l.color}1F`, color: l.color }}>
