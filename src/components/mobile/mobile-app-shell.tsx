...
import { clearOfficeExitPending, markOfficeExitPending, markOfficeWorkAck, readOfficeExitPending, useMobileTracking, } from './use-mobile-tracking';
...
const MobileTasksTab = dynamic(() => importWithRetry(() => import('./tabs/tasks-tab').then((m) => m.MobileTasksTab)), { ssr: false, loading: () => <TabLoading /> });
const MobileMessagesTab = dynamic(() => importWithRetry(() => import('./tabs/messages-tab').then((m) => m.MobileMessagesTab)), { ssr: false, loading: () => <TabLoading /> });
const MobileMoreTab = dynamic(() => importWithRetry(() => import('./tabs/more-tab').then((m) => m.MobileMoreTab)), { ssr: false, loading: () => <TabLoading /> });
const MobileScannerScreen = dynamic(() => importWithRetry(() => import('./scanner-screen').then((m) => m.MobileScannerScreen)), { ssr: false });
const MobileSafetyHub = dynamic(() => importWithRetry(() => import('./safety/safety-hub').then((m) => m.MobileSafetyHub)), { ssr: false });
const MobileEmergencySos = dynamic(() => importWithRetry(() => import('./safety/emergency-sos').then((m) => m.MobileEmergencySos)), { ssr: false });
const MobileHomePin = dynamic(() => importWithRetry(() => import('./safety/home-pin').then((m) => m.MobileHomePin)), { ssr: false });
const MobileTripHistory = dynamic(() => importWithRetry(() => import('./safety/trip-history').then((m) => m.MobileTripHistory)), { ssr: false });
const EmployeeDashboard = dynamic(() => importWithRetry(() => import('@/components/ui/employee-dashboard').then((m) => m.EmployeeDashboard)), { ssr: false });
...
export function MobileAppShell({ employee, onLogout, onEmployeeUpdate }: Props) {
    ...
    const clearLeaveTimers = useCallback(() => {
        ...
        if (leaveTickRef.current)
            window.clearInterval(leaveTickRef.current);
                ...
                subscribeOfficeExitPush(() => {
                    void (async () => {
                        try {
                            ...
                            const offices = await (await import('@/lib/mobile-api')).apiGet<{
                                offices?: any[];
    ...
    return (<div className="mobile-flutter-shell fixed inset-0 z-40 flex flex-col bg-[#F0F3FF] text-[#0F172A] antialiased" style={{
            fontFamily: 'var(--font-inter), Inter, ui-sans-serif, system-ui, -apple-system, sans-serif',
        }}>
			
			<div className="shrink-0 bg-[#0047FF]" style={{ height: 'max(env(safe-area-inset-top, 0px), 0px)' }} aria-hidden/>

			{installHint ? (<div className="z-50 flex items-start gap-2 bg-[#0047FF] px-3 py-2 text-[12px] font-medium text-white">
					...
					<button type="button" className="shrink-0 underline" onClick={() => {
			...
			{!installHint && pushHint ? (<div className="z-50 flex items-start gap-2 bg-[#0F172A] px-3 py-2 text-[12px] font-medium text-white">
			...
			{arriveOfficeHint ? (<div className="z-50 flex items-start gap-2 bg-[#0F172A] px-3 py-2.5 text-[12px] font-medium text-white">
					<p className="min-w-0 flex-1">You are at the office — scan the QR code to check in.</p>
					<button type="button" className="shrink-0 rounded-md bg-[#0047FF] px-2.5 py-1 text-[11px] font-bold" onClick={() => {
			...
			{!locBannerDismissed &&
            (locStatus === 'denied' || locStatus === 'prompt' || locStatus === 'unsupported') ? (<div className="z-50 flex items-start gap-2 bg-amber-500 px-3 py-2 text-[12px] font-medium text-amber-950">
					<p className="min-w-0 flex-1">
                ...
                : 'Allow location for attendance geofence, leave-office alerts, and Girl Safety tracking.'}
					...
					{locStatus !== 'unsupported' ? (<button type="button" className="shrink-0 font-bold underline" onClick={() => void requestLocation()}>
							Enable
						</button>) : null}
			...
			<div className="relative min-h-0 flex-1">
				...
				{section === 'home' ? (<div className="h-full">
						<MobileHomeTab employee={employee} refreshToken={refreshToken} onOpenScanner={() => setScannerOpen(true)} onOpenProfile={() => openPanel('profile')} onOpenSafety={() => openPanel('safety')} onOpenPanel={openPanel} onEmployeeUpdate={onEmployeeUpdate}/>
					</div>) : null}
				{section === 'tasks' ? (<div className="h-full">
						<MobileTasksTab employee={employee}/>
					</div>) : null}
				{section === 'messages' ? (<div className="h-full">
						<MobileMessagesTab employee={employee} onChatOpenChange={(open) => setMessagesChatOpen(open)} closeChatSignal={closeChatSignal}/>
					</div>) : null}
				{section === 'more' ? (<div className="h-full">
						<MobileMoreTab employee={employee} onOpenPanel={openPanel} onLogout={onLogout} onProfile={() => openPanel('profile')}/>
					</div>) : null}
			</div>

			<CorpBottomNav section={section} hidden={hideNav} onHome={goHome} onTasks={() => {
            setMessagesChatOpen(false);
            setCloseChatSignal((n) => n + 1);
            setPanelStack([]);
            setScannerOpen(false);
			...
			{scannerOpen ? (<MobileScannerScreen onClose={(ok) => {
                setScannerOpen(false);
                if (ok) {
                    setSection('home');
                    setRefreshToken((n) => n + 1);
			...
			{panelOpen && panel ? (<div className="fixed inset-0 z-[70] flex flex-col bg-[#F0F3FF]">
					<div className="flex items-center gap-2 border-b border-[#E2E8F0] bg-white px-2 pb-2.5" style={{ paddingTop: 'max(10px, env(safe-area-inset-top, 0px))' }}>
						<button type="button" onClick={popPanel} className="rounded-lg px-3 py-2 text-sm font-semibold text-[#0047FF]">
							← Back
						</button>
					...
					<div className="min-h-0 flex-1 overflow-y-auto">
						{panel === 'safety' ? (<MobileSafetyHub employee={employee} onOpen={openPanel}/>) : null}
						{panel === 'sos' ? <MobileEmergencySos employee={employee}/> : null}
						{panel === 'home_pin' ? (<MobileHomePin employee={employee} onEmployeeUpdate={onEmployeeUpdate}/>) : null}
						{panel === 'trips' ? <MobileTripHistory employee={employee}/> : null}
						{workTab ? (<div className="emp-mobile-panel h-full">
								<Suspense fallback={<div className="flex justify-center py-16">
									...
									<EmployeeDashboard employee={employee} onLogout={onLogout} onEmployeeUpdate={onEmployeeUpdate} mobilePanelTab={workTab as any} mobileLogsOnly={workTab === 'attendance'}/>
								</Suspense>
							</div>) : null}
					</div>
				</div>) : null}

			{leaveOpen ? (<div className="fixed inset-0 z-[90] flex items-end bg-black/45 sm:items-center sm:justify-center">
                ...
                : 'Outside office — choose within 5 min. Office work stays checked in. Going home checks you out.'}
						...
						<div className="mt-4 flex flex-col gap-2">
							<button type="button" disabled={leaveBusy} onClick={() => void handleLeaveChoice('office_work')} className="rounded-xl border border-[#E2E8F0] py-3 text-sm font-semibold text-[#0F172A]">
								Office work
							</button>
							<button type="button" disabled={leaveBusy} onClick={() => void handleLeaveChoice('going_home')} className="rounded-xl bg-[#0047FF] py-3 text-sm font-semibold text-white">
								Going home
							</button>
						</div>
					</div>
