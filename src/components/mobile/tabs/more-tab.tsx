'use client';
import { Building2, CalendarDays, ChevronRight, FileText, History, Home, IdCard, Leaf, LogOut, Route, Shield, Siren, TrendingUp, UserRound, } from 'lucide-react';
    ...
    const Section = ({ title, items }: {
        ...
        items: Item[];
            ...
            return (<div key={item.key}>
						<button type="button" onClick={() => (item.key === 'profile' ? onProfile() : onOpenPanel(item.key))} className="flex w-full items-center gap-3 px-3 py-3 text-left">
							<span className="flex size-9 items-center justify-center rounded-[10px]" style={{ backgroundColor: `${item.color}1F`, color: item.color }}>
						...
						{i < items.length - 1 ? (<div className="ml-[60px] h-px bg-[#E2E8F0]"/>) : null}
					</div>);
        })}
		</div>);
    return (<div className="flex h-full min-h-0 flex-col bg-[#F0F3FF]">
			<CorpPageHeader employee={employee} subtitle="Leaves, logs, profile & tools" onProfile={onProfile}/>
			<div className="min-h-0 flex-1 overflow-y-auto px-4 pb-[calc(172px+env(safe-area-inset-bottom,0px))] pt-4">
				...
				<button type="button" onClick={onLogout} className="mt-4 flex w-full items-center justify-center gap-2 rounded-[14px] border border-[#FEE4E2] bg-white py-3.5 text-sm font-semibold text-[#B42318]">
