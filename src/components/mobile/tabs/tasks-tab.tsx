    ...
    return (<div className="flex h-full min-h-0 flex-col bg-[#F0F3FF]">
			...
			<div className="min-h-0 flex-1 overflow-y-auto px-4 pb-[calc(172px+env(safe-area-inset-bottom,0px))] pt-4">
				...
				{loading ? (<div className="flex justify-center py-16">
					...
					</div>) : tasks.length === 0 ? (<div className="rounded-[14px] border border-[#E2E8F0] bg-white py-12 text-center text-sm text-[#64748B]">
						No tasks are currently allocated to you.
					</div>) : (<ul className="space-y-3">
						{tasks.map((t) => (<li key={t.id}>
								<button type="button" disabled={busyId === t.id} onClick={() => void cycleStatus(t)} className="w-full rounded-[14px] border border-[#E2E8F0] bg-white p-4 text-left active:scale-[0.99]">
									<div className="flex items-start justify-between gap-2">
										<p className="text-[15px] font-bold text-[#0F172A]">
											{t.title || t.name || 'Task'}
										</p>
										<span className={cn('shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-bold', statusColor(String(t.status || 'Pending')))}>
											{t.status || 'Pending'}
										</span>
									</div>
									{t.description ? (<p className="mt-1 line-clamp-2 text-[13px] text-[#64748B]">
											{t.description}
										</p>) : null}
									{t.dueDate ? (<p className="mt-2 text-[12px] font-medium text-[#94A3B8]">
