...
import { CalendarDays, ChevronRight, CircleCheck, Clock, FileText, Leaf, LogOut, QrCode, Siren, TrendingUp, } from 'lucide-react';
...
import { clockOut, getCurrentAttendanceStatus, getEventsForEmployee, getOpenSosIncidents, startGoingHomeTrip, } from '@/app/admin/actions';
...
export function MobileHomeTab({ employee, refreshToken, onOpenScanner, onOpenProfile, onOpenSafety, onOpenPanel, onEmployeeUpdate, }: Props) {
    ...
    const onShift = Boolean(today?.checkIn && (!today?.checkOut || String(today.checkOut).trim() === ''));
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            try {
                ...
                if (alive)
                    setOpenSos(Array.isArray(rows) ? rows : []);
            }
            catch {
            }
        };
    ...
    const handleClockOut = async () => {
        if (clockBusy)
            return;
        const ok = window.confirm(female
        ...
        setClockBusy(true);
        try {
            const token = localStorage.getItem('wrkspace_employee_token') ||
    ...
    return (<div className="flex h-full min-h-0 flex-col bg-[#F0F3FF]">
			<CorpPageHeader employee={employee} subtitle={onShift ? 'On shift · have a productive day' : 'Your workplace dashboard'} onProfile={onOpenProfile}/>
			<div className="min-h-0 flex-1 overflow-y-auto px-4 pt-4" style={{ paddingBottom: 'calc(172px + env(safe-area-inset-bottom, 0px))' }}>
				{toast ? (<div className="mb-3 rounded-xl bg-[#0047FF] px-3 py-2 text-center text-sm font-semibold text-white">
						{toast}
					</div>) : null}
				...
				<section className="rounded-[14px] border border-[#E2E8F0] bg-white p-4">
					<div className="flex items-start gap-2.5">
						...
						<div className="min-w-0 flex-1">
							...
							<p className={cn('text-[13px] font-semibold', onShift ? 'text-[#067647]' : 'text-[#64748B]')}>
								{onShift ? 'On shift' : today?.checkIn ? 'Checked out' : 'Not checked in'}
							</p>
						</div>
						<button type="button" className="text-[13px] font-semibold text-[#0047FF]" onClick={() => onOpenPanel?.('attendance')}>
							Logs
						</button>
					</div>
					...
					<div className="mt-3.5 space-y-2">
						{onShift ? (<>
								<button type="button" disabled={clockBusy} onClick={() => void handleClockOut()} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#0047FF] py-3.5 text-sm font-semibold text-white disabled:opacity-60">
									<LogOut className="size-4"/>
									{clockBusy ? 'Checking out…' : 'Check out'}
								</button>
								<button type="button" onClick={onOpenScanner} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#0047FF] py-3 text-sm font-semibold text-[#0047FF]">
									<QrCode className="size-4"/>
									Scan QR again
								</button>
							</>) : (<button type="button" onClick={onOpenScanner} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#0047FF] py-3.5 text-sm font-semibold text-white">
								<QrCode className="size-4"/>
				...
				{openSos.length > 0 ? (<button type="button" onClick={() => onOpenPanel?.('safety')} className="mt-4 flex w-full items-center gap-3 rounded-[14px] border border-rose-200 bg-rose-50 px-3.5 py-3.5 text-left">
				...
				{female ? (<section className="mt-4 rounded-[14px] border border-[#E2E8F0] bg-white p-4">
						<div className="flex items-center gap-2.5">
							<img src="/branding/girl-safety-logo.png" alt="" className="size-9 object-contain" onError={(e) => {
                (e.target as HTMLImageElement).src = '/branding/girl-safety-logo-64.png';
						...
						<div className="mt-3 flex gap-2">
							<button type="button" onClick={onOpenSafety} className="flex-1 rounded-xl border border-[#9D174D] py-3 text-sm font-semibold text-[#9D174D]">
								Open safety
							</button>
							<button type="button" onClick={() => onOpenPanel?.('sos')} className="flex-1 rounded-xl bg-[#B42318] py-3 text-sm font-semibold text-white">
								SOS
							</button>
						</div>
					</section>) : null}
				...
				<section className="mt-4 rounded-[14px] border border-[#E2E8F0] bg-white p-2">
					{loading && !overview ? (<div className="flex justify-center py-8">
							<div className="size-5 animate-spin rounded-full border-2 border-[#0047FF] border-t-transparent"/>
						</div>) : (<div className="grid grid-cols-2 gap-2">
							{metrics.map((m) => {
                const Icon = m.icon;
                return (<button key={m.label} type="button" onClick={() => onOpenPanel?.(m.label === 'Tasks'
                        ? 'tasks'
                        : m.label === 'Leaves'
                            ? 'leaves'
                            : m.label === 'Events'
                                ...
                                : m.label === 'Submissions'
                                    ...
                                    : 'attendance')} className="rounded-[10px] bg-[#F0F3FF] px-2.5 py-2.5 text-left">
										...
										{m.sub ? (<p className="text-[10px] font-medium text-[#64748B]">{m.sub}</p>) : null}
									</button>);
            })}
						</div>)}
				</section>
				...
				<section className="mt-4 rounded-[14px] border border-[#E2E8F0] bg-white p-4">
					<div className="mb-2 flex items-center justify-between">
						...
						<button type="button" className="text-[13px] font-semibold text-[#0047FF]" onClick={() => onOpenPanel?.('events')}>
							See all
						</button>
					</div>
					{events.length === 0 ? (<p className="py-3 text-[13.5px] text-[#64748B]">No upcoming events right now.</p>) : (<ul className="divide-y divide-[#E2E8F0]">
							{events.map((ev: any) => (<li key={ev.id}>
									<button type="button" onClick={() => onOpenPanel?.('events')} className="flex w-full items-center gap-2 py-2.5 text-left">
										<div className="min-w-0 flex-1">
