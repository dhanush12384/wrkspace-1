...
const OFFICE_WATCH_MS = 60000;
const HOME_WATCH_MS = 25000;
...
export function markOfficeExitPending() {
    try {
        sessionStorage.setItem(OFFICE_EXIT_KEY, JSON.stringify({ at: Date.now(), status: 'pending' }));
    }
    catch {
    }
}
...
export function readOfficeExitPending(): {
    at: number;
} | null {
    try {
        ...
        if (!parsed?.at || parsed.status === 'done')
            return null;
...
export function useMobileTracking({ employee, enabled, onLeaveOffice, onBackInsideOffice, onArriveOffice, onLocationError, }: Opts) {
    ...
    useEffect(() => {
        ...
        const loadOffices = async (force = false) => {
            const now = Date.now();
            if (!force && now - lastOfficesFetch.current < 5 * 60000 && officesRef.current.length) {
                return;
            }
            try {
        ...
        const tickOffice = async () => {
            ...
            try {
                ...
                const onShift = Boolean(att?.checkIn && (!att?.checkOut || String(att.checkOut).trim() === ''));
                ...
                const nearest = offices
                    ...
                    r: exitRadiusM(o),
                    checkInR: Number.isFinite(o.radiusMeters) && (o.radiusMeters as number) > 0
                ...
                const confirmed = wasInsideExit.current && outsideStreak.current >= OUTSIDE_CONFIRM_TICKS;
        ...
        const tickHome = async () => {
            if (!alive || !isFemaleEmployee(employee))
                return;
            try {
                const trips = await apiGet<{
                    trips?: any[];
                    trip?: any;
                }>('/api/safety/trips/home');
                const open = trips.trip ||
        ...
        return () => {
            alive = false;
            if (officeTimer)
                window.clearInterval(officeTimer);
            if (homeTimer)
    ...
    useEffect(() => {
        ...
        const postFix = async (lat: number, lng: number) => {
            if (!alive || !shouldTrack)
                return;
            const now = Date.now();
            if (now - lastPostedAt < 8000)
                return;
            lastPostedAt = now;
            try {
        ...
        const startWatch = () => {
            if (!alive || !shouldTrack || !navigator.geolocation)
                return;
            if (watchId != null)
                return;
        ...
        const refreshFlag = async () => {
            ...
            try {
                const st = await apiGet<{
                    shouldTrack?: boolean;
                }>('/api/attendance/live-track-status');
        ...
        void refreshFlag();
        statusTimer = window.setInterval(() => void refreshFlag(), 10000);
        return () => {
            alive = false;
            if (statusTimer)
