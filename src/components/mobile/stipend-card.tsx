...
export function StipendCard({ employee, onEmployeeUpdate, variant = 'mobile' }: Props) {
    ...
    if (amount == null || !Number.isFinite(amount))
        return null;
    const complete = Boolean(emp?.paymentDetailsComplete) || isPaymentDetailsComplete(emp || {});
    ...
    return (<section className={cn(dark
            ...
            : 'text-[15px] font-semibold text-[#0047FF]')}>
						...
						<span className={cn(dark ? 'ml-1.5 text-sm font-medium text-zinc-400' : 'font-semibold text-[#0047FF]')}>
							/ month
						</span>
					</p>
					{complete ? (<p className={cn('mt-1.5 text-xs font-medium', dark ? 'text-emerald-400' : 'text-[#067647]')}>
							Payment details on file
						</p>) : (<p className={cn('mt-1.5 text-xs', dark ? 'text-zinc-400' : 'text-[#64748B]')}>
							Add UPI or bank details for payout
						</p>)}
				</div>
			</div>
			{!complete ? (<button type="button" onClick={() => setOpen(true)} className={cn('mt-4 w-full py-3 text-sm font-semibold text-white', dark
                ? 'rounded-none bg-brand-600 hover:bg-brand-500 transition-colors'
                : 'rounded-xl bg-[#0047FF]')}>
					Update bank details
				</button>) : null}

			{open ? (<div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/60 p-4 sm:items-center">
					<div className={cn('w-full max-w-md p-5 shadow-xl', dark
                ...
                : 'rounded-2xl bg-white text-[#0F172A]')}>
						...
						<div className="mt-3 flex gap-2">
							<button type="button" onClick={() => setMode('upi')} className={cn('flex-1 py-2 text-sm font-semibold', dark ? 'rounded-none' : 'rounded-lg', mode === 'upi'
                ? dark
                ...
                : dark
                    ...
                    : 'bg-[#F1F5F9] text-[#334155]')}>
							...
							<button type="button" onClick={() => setMode('bank')} className={cn('flex-1 py-2 text-sm font-semibold', dark ? 'rounded-none' : 'rounded-lg', mode === 'bank'
                ? dark
                    ? 'bg-brand-600 text-white'
                    : 'bg-[#0047FF] text-white'
                : dark
                    ...
                    : 'bg-[#F1F5F9] text-[#334155]')}>
						...
						<div className="mt-3 space-y-2">
							{mode === 'upi' ? (<input value={upiId} onChange={(e) => setUpiId(e.target.value)} placeholder="name@upi" className={inputClass}/>) : (<>
									<input value={bankAccountHolderName} onChange={(e) => setBankAccountHolderName(e.target.value)} placeholder="Account holder name (exact as in bank)" className={inputClass}/>
									<input value={bankAccountNumber} onChange={(e) => setBankAccountNumber(e.target.value)} placeholder="Account number" className={inputClass} inputMode="numeric"/>
									<input value={bankAccountNumberConfirm} onChange={(e) => setBankAccountNumberConfirm(e.target.value)} placeholder="Re-enter account number" className={inputClass} inputMode="numeric"/>
									<input value={bankName} onChange={(e) => setBankName(e.target.value)} placeholder="Bank name" className={inputClass}/>
									<input value={bankIfsc} onChange={(e) => setBankIfsc(e.target.value)} placeholder="IFSC" className={inputClass}/>
								</>)}
						</div>
						{err ? (<p className={cn('mt-2 text-sm', dark ? 'text-red-400' : 'text-[#B42318]')}>{err}</p>) : null}
						<div className="mt-4 flex gap-2">
							<button type="button" onClick={() => setOpen(false)} className={cn('flex-1 py-2.5 text-sm font-semibold', dark
                ? 'rounded-none border border-zinc-700 text-zinc-300 hover:bg-zinc-800'
                : 'rounded-xl border border-[#E2E8F0]')}>
								Cancel
							</button>
							<button type="button" disabled={busy} onClick={() => void submit()} className={cn('flex-1 py-2.5 text-sm font-semibold text-white disabled:opacity-60', dark ? 'rounded-none bg-brand-600 hover:bg-brand-500' : 'rounded-xl bg-[#0047FF]')}>
								{busy ? 'Saving…' : 'Save'}
							</button>
						</div>
					</div>
