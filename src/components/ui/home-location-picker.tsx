...
export function HomeLocationPicker({ employee, onSaved }: Props) {
    ...
    if (!isFemale) {
        return (<div className="bg-white dark:bg-zinc-900 border border-black/[0.06] dark:border-white/[0.08] p-5 rounded-xl shadow-xs">
				<p className="text-sm text-slate-500 dark:text-zinc-400">Home location setup is part of Girl Safety (female employees).</p>
			</div>);
    }
    if (hasHome && !canEdit) {
        return (<div className="bg-white dark:bg-zinc-900 border border-black/[0.06] dark:border-white/[0.08] p-5 rounded-xl shadow-xs space-y-3">
				...
				{employee.homePlusCode ? (<p className="text-sm font-mono text-slate-700 dark:text-zinc-300">Plus Code: {employee.homePlusCode}</p>) : null}
				...
				<a href={googleMapsPinUrl(Number(employee.homeLat), Number(employee.homeLng))} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center rounded-md border border-black/[0.1] dark:border-white/[0.1] bg-slate-50 dark:bg-zinc-800 hover:bg-slate-100 dark:hover:bg-zinc-700 px-4 py-2 text-xs font-bold text-slate-700 dark:text-zinc-200 no-underline transition-colors cursor-pointer">
					View pin in Maps
				</a>
			</div>);
    }
    return (<div className="bg-white dark:bg-zinc-900 border border-black/[0.06] dark:border-white/[0.08] p-5 rounded-xl shadow-xs space-y-4">
			<div>
				<h2 className="text-lg font-black text-slate-900 dark:text-white">Home location</h2>
			...
			<div className="flex flex-wrap gap-2">
				<button type="button" onClick={useGps} disabled={busy} className="bg-[#E61E32] hover:bg-[#c9182a] text-white text-xs font-bold px-4 py-2 rounded-md transition-colors cursor-pointer disabled:opacity-50">
					{busy ? 'Working…' : 'Use current GPS'}
				</button>
				<a href={mapsHref} target="_blank" rel="noreferrer" className="bg-slate-800 dark:bg-zinc-800 hover:bg-slate-700 dark:hover:bg-zinc-700 text-white text-xs font-bold px-4 py-2 rounded-md inline-flex items-center no-underline transition-colors cursor-pointer">
					Open Google Maps
				</a>
				{lat != null && lng != null && (<a href={googleMapsPinUrl(lat, lng)} target="_blank" rel="noreferrer" className="border border-black/[0.1] dark:border-white/[0.1] bg-slate-50 dark:bg-zinc-850 hover:bg-slate-100 dark:hover:bg-zinc-750 text-slate-700 dark:text-zinc-200 text-xs font-bold px-4 py-2 rounded-md no-underline transition-colors cursor-pointer">
						Verify pin in Maps
					</a>)}
			</div>
			...
			<div className="space-y-1">
				...
				<div className="flex gap-2">
					<input value={paste} onChange={(e) => setPaste(e.target.value)} placeholder="https://maps.google.com/... or 12.9716,77.5946" className="flex-1 border border-black/[0.1] dark:border-white/[0.1] bg-slate-50 dark:bg-zinc-950 px-3 py-2 text-sm text-slate-900 dark:text-white rounded-md focus:outline-none focus:ring-1 focus:ring-[#E61E32]/40"/>
					<button type="button" onClick={applyPaste} className="bg-slate-800 dark:bg-zinc-800 hover:bg-slate-700 dark:hover:bg-zinc-700 text-white text-xs font-bold px-4 rounded-md transition-colors cursor-pointer">
						Apply
					</button>
				</div>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
				<div className="space-y-1">
					...
					<input value={plus} readOnly className="w-full border border-black/[0.1] dark:border-white/[0.1] bg-slate-50 dark:bg-zinc-950 px-3 py-2 text-sm font-mono text-slate-700 dark:text-zinc-300 rounded-md outline-none"/>
				</div>
				<div className="space-y-1">
					<label className="text-[10px] uppercase font-bold text-slate-500 dark:text-zinc-400">Note (optional)</label>
					<input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Apartment / landmark" className="w-full border border-black/[0.1] dark:border-white/[0.1] bg-slate-50 dark:bg-zinc-950 px-3 py-2 text-sm text-slate-900 dark:text-white rounded-md focus:outline-none focus:ring-1 focus:ring-[#E61E32]/40"/>
				</div>
			</div>

			{lat != null && lng != null && (<p className="text-xs font-mono text-slate-500 dark:text-zinc-400">
			...
			<button type="button" disabled={busy || lat == null} onClick={save} className="w-full bg-[#E61E32] hover:bg-[#c9182a] text-white font-bold py-3 px-6 rounded-md transition-all duration-200 shadow-sm cursor-pointer disabled:opacity-50 text-center text-sm">
