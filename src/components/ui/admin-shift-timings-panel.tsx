    ...
    return (<div className="space-y-6">
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				...
				<div className="flex items-center gap-2 flex-wrap">
					<button type="button" onClick={() => applyPresetToAll('09:30 AM', '06:30 PM')} className="text-xs px-3.5 py-1.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-medium cursor-pointer shadow-2xs transition-all">
						Preset: 9:30 AM – 6:30 PM
					</button>
					<button type="button" onClick={() => void load()} className="p-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-900 transition-all rounded-xl cursor-pointer shadow-2xs" title="Refresh Shift Timings">
			...
			<div className="flex items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-slate-200/90 shadow-2xs">
				<div className="relative flex-1 max-w-md">
					...
					<input type="text" placeholder="Search employee by name, ID, wing, or email..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#E61E32]/20 focus:border-[#E61E32]/40 transition-colors"/>
				</div>
			...
			{loading ? (<div className="bg-white border border-slate-200/90 rounded-2xl p-12 text-center shadow-2xs">
				...
				</div>) : filteredRows.length === 0 ? (<div className="text-center py-16 bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs">
					...
					<table className="w-full text-left text-xs border-collapse">
						...
						<tbody className="divide-y divide-slate-100 text-slate-700">
							{filteredRows.map((e) => (<tr key={e.id} className="hover:bg-slate-50/70 transition-colors">
									...
									<td className="py-3.5 px-4">
										<div className="flex items-center gap-1.5 max-w-[160px]">
											...
											<input value={draft[e.id]?.in ?? ''} onChange={(ev) => setDraft((prev) => ({
                    ...prev,
                    [e.id]: { in: ev.target.value, out: prev[e.id]?.out ?? '' },
                }))} placeholder="Open (any time)" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#E61E32]/20 focus:border-[#E61E32]/40 transition-colors"/>
										</div>
									</td>
									<td className="py-3.5 px-4">
										<div className="flex items-center gap-1.5 max-w-[160px]">
											<ClockIcon className="size-3.5 text-slate-400 shrink-0"/>
											<input value={draft[e.id]?.out ?? ''} onChange={(ev) => setDraft((prev) => ({
                    ...prev,
                    [e.id]: { in: prev[e.id]?.in ?? '', out: ev.target.value },
                }))} placeholder="07:30 PM default" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#E61E32]/20 focus:border-[#E61E32]/40 transition-colors"/>
										</div>
									</td>
									<td className="py-3.5 px-4 text-right whitespace-nowrap">
										<button type="button" disabled={savingId === e.id} onClick={() => void save(e.id)} className="inline-flex items-center gap-1.5 bg-[#E61E32] hover:bg-[#c9182a] disabled:opacity-50 text-white text-xs font-semibold px-3.5 py-1.5 rounded-xl cursor-pointer transition-all shadow-2xs">
