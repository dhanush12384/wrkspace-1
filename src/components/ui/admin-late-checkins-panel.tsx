...
export function AdminLateCheckinsPanel({ adminEmail }: AdminLateCheckinsPanelProps) {
    ...
    return (<div className="space-y-6">
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				...
				<div className="flex items-center gap-2 flex-wrap">
					<div className="flex items-center gap-1 bg-slate-100/90 p-1 rounded-xl border border-slate-200/80">
						<button type="button" onClick={() => setFilter('PENDING')} className={cn("text-xs px-3.5 py-1.5 rounded-lg font-medium transition-all cursor-pointer", filter === 'PENDING'
            ? "bg-[#E61E32] text-white shadow-xs font-semibold"
            : "text-slate-600 hover:text-slate-900 hover:bg-white/80")}>
							Pending ({filter === 'PENDING' ? rows.length : pendingCount})
						</button>
						<button type="button" onClick={() => setFilter('ALL')} className={cn("text-xs px-3.5 py-1.5 rounded-lg font-medium transition-all cursor-pointer", filter === 'ALL'
            ? "bg-[#E61E32] text-white shadow-xs font-semibold"
            : "text-slate-600 hover:text-slate-900 hover:bg-white/80")}>
							All Requests
						</button>
					</div>
					<button type="button" onClick={() => void load()} className="p-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-900 transition-all rounded-xl cursor-pointer shadow-2xs" title="Refresh Requests">
			...
			{loading ? (<div className="bg-white border border-slate-200/90 rounded-2xl p-12 text-center shadow-2xs">
				...
				</div>) : rows.length === 0 ? (<div className="text-center py-16 bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs">
                ...
                return (<div key={r.id} className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-all space-y-3.5">
								<div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
									...
									<div className="shrink-0">
										<span className={cn("inline-flex items-center gap-1.5 text-xs font-medium px-3.5 py-1.5 rounded-full border shadow-2xs tracking-wide uppercase", isPending && "bg-amber-50 text-amber-800 border-amber-300", isApproved && "bg-emerald-50 text-emerald-800 border-emerald-300", isDenied && "bg-rose-50 text-rose-800 border-rose-300", !isPending && !isApproved && !isDenied && "bg-slate-50 text-slate-700 border-slate-300")}>
											<span className={cn("size-2 rounded-full", isPending ? "bg-amber-500" : isApproved ? "bg-emerald-500" : "bg-rose-500")}/>
											{r.status}
										</span>
									</div>
								</div>
								...
								{r.reason ? (<div className="bg-slate-50/70 border border-slate-200/80 rounded-xl p-3.5 text-xs text-slate-700 leading-relaxed font-normal">
										<span className="font-semibold text-slate-800">Reason Provided: </span>
										"{r.reason}"
									</div>) : (<div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 text-xs text-slate-400 italic">
										No explanation text provided.
									</div>)}

								
								{isPending && (<div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
										<button type="button" disabled={busyId === r.id} onClick={() => void act(r.id, 'approve')} className="inline-flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-xs font-semibold px-4 py-2 rounded-xl cursor-pointer transition-all shadow-xs" style={{ backgroundColor: '#059669', color: '#ffffff' }}>
											<CheckCircleIcon className="size-3.5 text-white"/>
											{busyId === r.id ? 'Processing…' : 'Approve Permission'}
										</button>
										<button type="button" disabled={busyId === r.id} onClick={() => void act(r.id, 'deny')} className="inline-flex items-center gap-1.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white text-xs font-semibold px-4 py-2 rounded-xl cursor-pointer transition-all shadow-xs" style={{ backgroundColor: '#e11d48', color: '#ffffff' }}>
