...
export function ChatAvatar({ id, name, photoUrl, hasPhoto = true, size = 32, className, adminEmail, shape = 'circle', }: Props) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (admin) {
                try {
                    const res = await fetch(`/api/admin/avatars/${encodeURIComponent(id)}?email=${encodeURIComponent(admin)}`);
                    if (res.ok) {
                        const blob = await res.blob();
                        if (blob.size > 50) {
                            objectUrl = URL.createObjectURL(blob);
    ...
    if (src) {
        return (<img src={src} alt={name} width={size} height={size} className={cn(shape === 'circle' ? 'rounded-full' : 'rounded-lg', 'object-cover shrink-0 bg-zinc-200', className)} style={{ width: size, height: size }} onError={() => {
                if (id) {
                    _cache.delete(id);
                    _failed.add(id);
                }
    ...
    return (<span className={cn(shape === 'circle' ? 'rounded-full' : 'rounded-lg', 'flex items-center justify-center font-bold shrink-0', className)} style={{
            width: size,
            height: size,
