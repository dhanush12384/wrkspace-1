...
import { loginEmployee, loginEmployeeWithGoogle, sendEmployeeOtp, verifyEmployeeOtpAndResetPassword, setEmployeeGender, } from '@/app/admin/actions';
import { firebaseAuth, googleProvider } from '@/lib/firebase-client';
import { signInWithPopup } from 'firebase/auth';
const MobileAppShell = dynamic(() => importWithRetry(() => import('@/components/mobile/mobile-app-shell').then((m) => {
    if (!m?.MobileAppShell)
        throw new Error('MobileAppShell export missing');
    return m.MobileAppShell;
})), { ssr: false, loading: () => <ShellLoading /> });
const EmployeeDashboard = dynamic(() => importWithRetry(() => import('./employee-dashboard').then((m) => {
    if (!m?.EmployeeDashboard)
        throw new Error('EmployeeDashboard export missing');
    return m.EmployeeDashboard;
})), { ssr: false, loading: () => <ShellLoading /> });
function ShellLoading() {
    return (<div className="flex min-h-screen items-center justify-center bg-white text-black">
			<div className="size-8 animate-spin rounded-full border-2 border-black border-t-transparent"/>
		</div>);
}
function EmployeeShell({ employee, onLogout, onEmployeeUpdate, }: {
    employee: any;
    onLogout: () => void;
    ...
    if (employee && hasSubmittedFeedback === false) {
        return (<UnanimousFormGate userEmail={employee.email} userName={`${employee.firstName} ${employee.lastName}`} userType="EMPLOYEE" userId={employee.id} onSuccess={() => setHasSubmittedFeedback(true)}/>);
    }
    if (isMobile === null || hasSubmittedFeedback === null)
        return <ShellLoading />;
    if (isMobile) {
        return (<SoftErrorBoundary>
				<Suspense fallback={<ShellLoading />}>
					<MobileAppShell employee={employee} onLogout={onLogout} onEmployeeUpdate={onEmployeeUpdate}/>
				</Suspense>
			</SoftErrorBoundary>);
    }
    return (<SoftErrorBoundary>
			<Suspense fallback={<ShellLoading />}>
				<EmployeeDashboard employee={employee} onLogout={onLogout} onEmployeeUpdate={onEmployeeUpdate}/>
			</Suspense>
		</SoftErrorBoundary>);
}
...
export function AuthPage() {
            ...
            try {
                ...
                if (saved) {
                    ...
                    if (res.success && res.employee) {
                        const existingToken = localStorage.getItem('wrkspace_employee_token') ||
                            String((parsed as {
                                token?: string;
                            }).token || '');
                        ...
                        if (existingToken)
                            localStorage.setItem('wrkspace_employee_token', existingToken);
        ...
        try {
            ...
            if (result.success && result.employee) {
                ...
                if ((result as any).token)
                    localStorage.setItem('wrkspace_employee_token', (result as any).token);
    ...
    if (loggedInEmployee && needsGender) {
        ...
        return (<main className="min-h-screen bg-white flex items-center justify-center p-6 font-sans text-black">
				<div className="w-full max-w-sm bg-white border border-black/10 rounded-xl p-6 space-y-4">
					...
					{message?.type === 'error' && (<p className="text-xs text-red-500 font-medium">{message.text}</p>)}
					<div className="grid grid-cols-2 gap-3 pt-1">
						<button type="button" onClick={() => pick('FEMALE')} className="bg-black text-white text-sm font-medium py-2.5 px-3 rounded-lg hover:bg-black/85 transition-colors">
							Female
						</button>
						<button type="button" onClick={() => pick('MALE')} className="bg-black/5 text-black text-sm font-medium py-2.5 px-3 rounded-lg hover:bg-black/10 transition-colors border border-black/15">
							Male
						</button>
					</div>
    ...
    if (loggedInEmployee) {
        return (<EmployeeShell employee={loggedInEmployee} onLogout={handleLogout} onEmployeeUpdate={(next) => {
                setLoggedInEmployee(next);
                try {
                    localStorage.setItem('wrkspace_employee_session', JSON.stringify(next));
                }
    ...
    const termsText = (<>
			...
			<a href="#" className="font-medium text-black/50 underline underline-offset-2 hover:text-black transition-colors">
				Terms and Services
			</a>{' '}
			and{' '}
			<a href="#" className="font-medium text-black/50 underline underline-offset-2 hover:text-black transition-colors">
    ...
    return (<section className="min-h-screen bg-white p-3 text-black antialiased [font-synthesis:none]">
			<div className="grid min-h-[calc(100vh-1.5rem)] gap-6 lg:grid-cols-[1.18fr_0.82fr] xl:grid-cols-[1.22fr_0.78fr]">
				<div className="flex min-h-[600px] items-center rounded-md border border-black/20 bg-white px-6 py-8 sm:px-10 lg:min-h-0 lg:px-12 lg:py-14">
					<div className="mx-auto w-full max-w-[510px]">
						...
						{view === 'login' && (<>
								<div className="mt-6 grid gap-3 sm:grid-cols-2">
									<SocialButton icon={<GoogleIcon />} label="Sign in with Google" onClick={handleGoogleLogin} disabled={isLoading}/>
									<SocialButton icon={<AppleIcon />} label="Sign in with Apple" onClick={() => setMessage({ type: 'error', text: 'Apple sign-in is coming soon.' })} disabled={isLoading}/>
								</div>
								...
								{message && (<div className={cn('mb-4 p-3 rounded-lg text-xs font-medium border', message.type === 'success'
                    ...
                    : 'bg-red-500/10 border-red-500/30 text-red-400')}>
								...
								<form onSubmit={handleLogin} className="space-y-4">
									<FieldBox label="Email" value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="your.email@example.com" required/>

									<div>
										<FieldBox label="Password" value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="••••••••••••" required/>
                ...
                setForgotMessage(null);
									...
									<button type="submit" disabled={isLoading} className="mt-6 flex h-11 w-full items-center justify-center rounded-[10px] border border-black/40 bg-black text-base font-semibold text-white transition-all hover:bg-black/85 disabled:opacity-50 cursor-pointer">
										{isLoading ? 'Processing...' : 'Submit'}
									</button>
								</form>
							</>)}

						{view === 'forgot' && (<div className="mt-6 space-y-4">
								<p className="text-black/60 text-xs sm:text-sm">
									Enter your registered email address to receive a password reset OTP code.
								</p>

								{forgotMessage && (<div className={cn('p-3 rounded-lg text-xs font-medium border', forgotMessage.type === 'success'
                    ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'
                    : 'bg-red-500/10 border-red-500/30 text-red-400')}>
										{forgotMessage.text}
									</div>)}
								...
								<form onSubmit={handleSendOtp} className="space-y-4">
									<FieldBox label="Registered Email" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} type="email" placeholder="your.email@example.com" required/>

									<button type="submit" disabled={isSending} className="mt-4 flex h-11 w-full items-center justify-center rounded-[10px] border border-black/40 bg-black text-base font-semibold text-white transition-all hover:bg-black/85 disabled:opacity-50 cursor-pointer">
										{isSending ? 'Sending OTP...' : 'Send OTP Code'}
									</button>

									<button type="button" onClick={goBackToLogin} className="w-full text-center text-xs font-medium text-black/50 hover:text-black transition-colors underline pt-1">
										Back to Sign In
									</button>
								</form>
							</div>)}

						{view === 'forgot_verify' && (<div className="mt-6 space-y-4">
								<p className="text-black/60 text-xs sm:text-sm">
                    ...
                    : 'bg-red-500/10 border-red-500/30 text-red-400')}>
								...
								<form onSubmit={handleVerifyAndReset} className="space-y-3">
									<FieldBox label="OTP Code" value={otpCode} onChange={(e) => setOtpCode(e.target.value)} type="text" placeholder="6-digit OTP" required/>

									<FieldBox label="New Password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} type="password" placeholder="••••••••••••" required/>

									<FieldBox label="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} type="password" placeholder="••••••••••••" required/>
						...
						{view === 'forgot_sent' && (<div className="mt-6 space-y-4 text-center">
								...
								<button type="button" onClick={goBackToLogin} className="mt-4 flex h-11 w-full items-center justify-center rounded-[10px] border border-black/40 bg-black text-base font-semibold text-white transition-all hover:bg-black/85 cursor-pointer">
									Continue to Sign In
								</button>
							</div>)}
					</div>
...
function SocialButton({ icon, label, onClick, disabled }: {
    ...
    return (<button type="button" onClick={onClick} disabled={disabled} className="flex h-10 items-center justify-center gap-2 rounded-[10px] border border-black/25 bg-white px-3 text-xs font-medium text-black transition-colors hover:bg-black/[0.03] disabled:opacity-50 sm:text-sm cursor-pointer">
			<span className="shrink-0">{icon}</span>
			<span className="whitespace-nowrap truncate">{label}</span>
		</button>);
}
function FieldBox({ label, value, type = 'text', placeholder, onChange, required, }: {
    label: string;
    value: string;
    type?: string;
    placeholder?: string;
    ...
    return (<label className="flex h-11 items-center justify-between gap-3 rounded-[10px] border border-black/25 bg-white px-4 text-sm leading-none">
			<input type={type} value={value} aria-label={label} placeholder={placeholder || label} required={required} onChange={onChange} className="min-w-0 flex-1 truncate bg-transparent text-black text-sm outline-none placeholder:text-black/30:text-white/35"/>
...
function CheckboxLine({ children }: {
    ...
    return (<label className="flex items-start gap-2.5 cursor-pointer">
			<span className="relative mt-0.5 size-3.5 shrink-0">
				<input type="checkbox" className="peer size-full appearance-none rounded-[2px] border border-black/25 bg-white checked:border-black checked:bg-black"/>
