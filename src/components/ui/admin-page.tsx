...
import { AtSignIcon, ChevronLeftIcon, Grid2x2PlusIcon, LockIcon, KeyIcon, } from 'lucide-react';
...
export function AdminPage() {
    ...
    if (formState === 'dashboard') {
        return (<AdminDashboard email={email} onLogout={() => {
                localStorage.removeItem('wrkspace_admin_session');
    ...
    return (<main className="dark relative md:h-screen md:overflow-hidden lg:grid lg:grid-cols-2 bg-zinc-950">
			...
			<div className="relative flex min-h-screen flex-col justify-center bg-zinc-950 p-8 lg:p-12 overflow-hidden">
				...
				<Button variant="ghost" className="absolute top-7 left-5 text-zinc-400 hover:text-white hover:bg-zinc-900/50 cursor-pointer transition-colors" asChild>
					<a href="/">
				...
				<div className="relative z-10 mx-auto w-full max-w-sm space-y-6">
					...
					{message && (<div className={cn("p-3 rounded-none text-xs border", message.type === 'success'
                ...
                : "bg-red-950/30 border-red-800 text-red-400")}>
					...
					{formState === 'login' && (<div className="space-y-6">
							...
							<form onSubmit={handleLogin} className="space-y-4">
								<div className="space-y-2">
									...
									<div className="relative h-max">
										<Input placeholder="your.email@example.com" className="peer ps-9 bg-zinc-900/50 border-zinc-800 text-zinc-100 placeholder:text-zinc-500 focus-visible:ring-brand-500 focus-visible:border-brand-500 transition-colors" type="email" required value={email} onChange={(e) => setEmail(e.target.value)}/>
								...
								<div className="space-y-2">
									<div className="flex items-center justify-between">
										...
										<Button type="button" variant="link" className="p-0 h-auto text-xs text-zinc-400 hover:text-brand-400 transition-colors font-normal hover:no-underline cursor-pointer" onClick={() => {
                setMessage(null);
                setFormState('forgot-request');
									...
									<div className="relative h-max">
										<Input placeholder="••••••••" className="peer ps-9 bg-zinc-900/50 border-zinc-800 text-zinc-100 placeholder:text-zinc-500 focus-visible:ring-brand-500 focus-visible:border-brand-500 transition-colors" type="password" required value={password} onChange={(e) => setPassword(e.target.value)}/>
										<div className="text-zinc-500 pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-3 peer-focus:text-brand-400 transition-colors">
								...
								<Button type="submit" disabled={isLoading} className="w-full h-11 bg-brand-600 hover:bg-brand-500 disabled:bg-brand-800 disabled:opacity-50 text-white font-medium shadow-md shadow-brand-600/20 hover:shadow-brand-600/30 transition-all duration-200 cursor-pointer">
							...
							<GoogleSignInButton onClick={handleGoogleLogin} disabled={isLoading} loading={isLoading} label="Continue with Google"/>
						</div>)}

					
					{formState === 'forgot-request' && (<div className="space-y-6">
							<div className="flex flex-col space-y-2">
								<h1 className="font-heading text-3xl font-bold tracking-tight text-white">
									Reset Password
								</h1>
							...
							<form onSubmit={handleRequestOtp} className="space-y-4">
								...
								<Button type="submit" disabled={isLoading} className="w-full bg-brand-600 hover:bg-brand-500 disabled:bg-brand-800 disabled:opacity-50 text-white font-medium shadow-md shadow-brand-600/20 hover:shadow-brand-600/30 transition-all duration-200 cursor-pointer">
									<span>{isLoading ? 'Sending OTP...' : 'Send OTP Email'}</span>
								</Button>

								<Button type="button" variant="ghost" className="w-full text-zinc-400 hover:text-white cursor-pointer" onClick={() => {
                setMessage(null);
                setFormState('login');
            }}>
									Back to Login
					...
					{formState === 'forgot-verify' && (<div className="space-y-6">
							<div className="flex flex-col space-y-2">
								<h1 className="font-heading text-3xl font-bold tracking-tight text-white">
									Enter OTP Code
								</h1>
							...
							<form onSubmit={handleResetPassword} className="space-y-4">
								<div className="space-y-2">
									...
									<div className="relative h-max">
										<Input placeholder="123456" className="peer ps-9 bg-zinc-900/50 border-zinc-800 text-zinc-100 placeholder:text-zinc-500 focus-visible:ring-brand-500 focus-visible:border-brand-500 transition-colors" type="text" required maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value)}/>
										<div className="text-zinc-500 pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-3 peer-focus:text-brand-400 transition-colors">
								...
								<div className="space-y-2">
									...
									<div className="relative h-max">
										<Input placeholder="••••••••" className="peer ps-9 bg-zinc-900/50 border-zinc-800 text-zinc-100 placeholder:text-zinc-500 focus-visible:ring-brand-500 focus-visible:border-brand-500 transition-colors" type="password" required value={newPassword} onChange={(e) => setNewPassword(e.target.value)}/>
										<div className="text-zinc-500 pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-3 peer-focus:text-brand-400 transition-colors">
											<LockIcon className="size-4" aria-hidden="true"/>
										</div>
									</div>
								...
								<div className="space-y-2">
									...
									<div className="relative h-max">
										<Input placeholder="••••••••" className="peer ps-9 bg-zinc-900/50 border-zinc-800 text-zinc-100 placeholder:text-zinc-500 focus-visible:ring-brand-500 focus-visible:border-brand-500 transition-colors" type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}/>
										<div className="text-zinc-500 pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-3 peer-focus:text-brand-400 transition-colors">
											<LockIcon className="size-4" aria-hidden="true"/>
										</div>
									</div>
...
function FloatingPaths({ position }: {
    ...
    return (<div className="pointer-events-none absolute inset-0">
			<svg className="h-full w-full text-white" viewBox="0 0 696 316" fill="none">
				...
				{paths.map((path) => (<motion.path key={path.id} d={path.d} stroke="currentColor" strokeWidth={path.width} strokeOpacity={0.06 + path.id * 0.015} initial={{ pathLength: 0.3, opacity: 0.5 }} animate={{
                pathLength: 1,
                opacity: [0.2, 0.5, 0.2],
                pathOffset: [0, 1, 0],
            }} transition={{
