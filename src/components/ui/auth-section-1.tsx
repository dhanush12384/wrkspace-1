...
const termsText = (<>
    ...
    <a href="#" className="font-medium text-black/50 underline underline-offset-2 dark:text-white/45 hover:text-black dark:hover:text-white transition-colors">
      Terms and Services
    </a>{" "}
    and{" "}
    <a href="#" className="font-medium text-black/50 underline underline-offset-2 dark:text-white/45 hover:text-black dark:hover:text-white transition-colors">
...
export default function AuthSectionOne() {
    return (<section className="min-h-screen bg-white p-3 text-black antialiased [font-synthesis:none] dark:bg-[#050505] dark:text-white">
          ...
          <div className="mx-auto w-full max-w-[510px]">
            ...
            <form className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2">
                {formFields.map((field) => (<FieldBox key={field.label} label={field.label} value={field.value}/>))}
              </div>
              ...
              <button type="button" className="mt-6 flex h-11 w-full items-center justify-center rounded-[10px] border border-black/40 bg-black text-base font-semibold text-white transition-all hover:bg-black/85 dark:border-white/40 dark:bg-white dark:text-black dark:hover:bg-white/85 cursor-pointer">
                Submit
              </button>
            </form>
          </div>
...
function SocialButton({ icon, label }: {
    ...
    return (<button type="button" className="flex h-10 items-center justify-center gap-2 rounded-[10px] border border-black/25 bg-white px-3 text-xs font-medium text-black transition-colors hover:bg-black/[0.03] dark:border-white/20 dark:bg-white/5 dark:text-white dark:hover:bg-white/10 sm:text-sm cursor-pointer">
...
function FieldBox({ label, value, type = "text", }: {
    ...
    return (<label className="flex h-11 items-center justify-between gap-3 rounded-[10px] border border-black/25 bg-white px-4 text-sm leading-none dark:border-white/15 dark:bg-white/5">
      <input type={type} value={inputValue} aria-label={label} onFocus={() => {
            if (!isEditing) {
                setInputValue("");
                setIsEditing(true);
            }
      ...
      {!isEditing && (<span className="shrink-0 text-black/50 text-xs font-medium dark:text-white/50">{label}</span>)}
    </label>);
}
function CheckboxLine({ children }: {
    ...
    return (<label className="flex items-start gap-2.5 cursor-pointer">
      <span className="relative mt-0.5 size-3.5 shrink-0">
        <input type="checkbox" className="peer size-full appearance-none rounded-[2px] border border-black/25 bg-white checked:border-black checked:bg-black dark:border-white/30 dark:bg-white/5 dark:checked:border-white dark:checked:bg-white"/>
