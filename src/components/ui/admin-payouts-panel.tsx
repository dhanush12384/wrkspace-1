...
export function AdminPayoutsPanel({ adminEmail }: AdminPayoutsPanelProps) {
    ...
    useEffect(() => {
        ...
        return connectRealtime({
            ...
            onAttendance: (p) => {
                ...
                if (action === 'payout-update' || action === 'payout_update')
                    void load(true);
            },
        });
    }, [rtToken, load]);
    ...
    const filteredRows = useMemo(() => {
        ...
        if (!q)
            return rows;
        return rows.filter(e => nameOf(e).toLowerCase().includes(q) ||
    ...
    return (<div className="space-y-6">
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				...
				<button type="button" onClick={() => void load()} className="p-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-900 transition-all rounded-xl cursor-pointer shadow-2xs self-start sm:self-auto" title="Refresh Payouts">
					<RefreshCwIcon className="size-4"/>
				</button>
			</div>

			
			{msg && (<div className={cn("p-3.5 rounded-xl text-xs flex items-center gap-2 border shadow-2xs", msg.type === 'success' ? "bg-emerald-50 text-emerald-800 border-emerald-200" : "bg-rose-50 text-rose-800 border-rose-200")}>
			...
			{notOnList.length > 0 && (<div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs space-y-3">
					...
					<div className="flex flex-wrap items-center gap-3">
						<div className="flex-1 min-w-[240px]">
							<select value={addId} onChange={(e) => setAddId(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 outline-none focus:ring-2 focus:ring-[#E61E32]/20 focus:border-[#E61E32]/40 transition-colors">
								<option value="">-- Choose Employee to Add --</option>
								{notOnList.map((e) => (<option key={e.id} value={e.id}>
										{nameOf(e)} ({e.id})
									</option>))}
							</select>
						</div>
						<div className="w-36 flex items-center bg-slate-50 border border-slate-200 rounded-xl px-3 py-2">
							...
							<input value={addAmount} onChange={(e) => setAddAmount(e.target.value)} placeholder="3500" className="w-full bg-transparent text-xs text-slate-800 outline-none font-medium"/>
						</div>
						<button type="button" disabled={!addId || savingId === addId} onClick={() => {
			...
			<div className="flex items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-slate-200/90 shadow-2xs">
				<div className="relative flex-1 max-w-md">
					...
					<input type="text" placeholder="Search by name, ID, email, UPI, or account..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#E61E32]/20 focus:border-[#E61E32]/40 transition-colors"/>
				</div>
					...
					<table className="w-full text-left text-xs border-collapse">
						...
						<tbody className="divide-y divide-slate-100 text-slate-700">
							{filteredRows.length === 0 ? (<tr>
								...
								</tr>) : (filteredRows.map((e) => (<Fragment key={e.id}>
										<tr className="hover:bg-slate-50/70 transition-colors">
											...
											<td className="py-3.5 px-4">
												<div className="flex items-center gap-1.5 max-w-[160px]">
													...
													<input value={draftAmount[e.id] ?? ''} onChange={(ev) => setDraftAmount((prev) => ({ ...prev, [e.id]: ev.target.value }))} className="w-20 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 text-xs text-slate-800 font-semibold focus:outline-none focus:ring-2 focus:ring-[#E61E32]/20"/>
													<button type="button" disabled={savingId === e.id} onClick={() => void saveAmount(e.id, draftAmount[e.id] === ''
                    ? null
                    : Math.round(Number(draftAmount[e.id])))} className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-[11px] font-semibold px-2.5 py-1 rounded-lg cursor-pointer transition-all shadow-2xs" style={{ backgroundColor: '#059669', color: '#ffffff' }}>
														Save
													</button>
												</div>
											</td>
											<td className="py-3.5 px-4 text-xs text-slate-600">
												{e.upiId ? (<span className="inline-flex items-center gap-1 bg-slate-100 text-slate-700 font-medium px-2 py-0.5 rounded-md border border-slate-200/60">
														<CreditCardIcon className="size-3 text-slate-400"/>
														UPI: {e.upiId}
													</span>) : e.bankAccountNumber ? (<span className="inline-flex items-center gap-1 bg-slate-100 text-slate-700 font-medium px-2 py-0.5 rounded-md border border-slate-200/60">
														<BuildingIcon className="size-3 text-slate-400"/>
														{e.bankName || 'Bank'} · ****{String(e.bankAccountNumber).slice(-4)}
													</span>) : (<span className="text-slate-400 italic">No account provided</span>)}
											</td>
											<td className="py-3.5 px-4">
												<span className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-full border shadow-2xs", e.paymentDetailsComplete
                    ...
                    : "bg-amber-50 text-amber-700 border-amber-200")}>
											...
											<td className="py-3.5 px-4 text-right whitespace-nowrap">
												<div className="inline-flex items-center justify-end gap-1.5">
													<button type="button" onClick={() => setExpanded(expanded === e.id ? null : e.id)} className="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 cursor-pointer shadow-2xs transition-all">
														<PencilIcon className="size-3 text-slate-400"/>
														{expanded === e.id ? 'Close' : 'Bank Info'}
													</button>
													<button type="button" disabled={savingId === e.id} onClick={() => void saveAmount(e.id, null)} className="p-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg cursor-pointer transition-all shadow-2xs disabled:opacity-50" style={{ backgroundColor: '#dc2626', color: '#ffffff' }} title="Remove Recipient">
														<Trash2Icon className="size-3.5 text-white"/>
													</button>
												</div>
											</td>
										</tr>
										{expanded === e.id && (<tr className="bg-slate-50/80 border-b border-slate-200/80">
                        ...
                        ['bankName', 'Bank Name'],
                        ['bankIfsc', 'IFSC Code'],
                    ] as const).map(([key, label]) => (<label key={key} className="flex flex-col gap-1 text-[11px] font-medium text-slate-600">
																	{label}
																	<input value={payDraft[e.id]?.[key] || ''} onChange={(ev) => setPayDraft((prev) => ({
                            ...prev,
                            [e.id]: {
                                ...(prev[e.id] || emptyPay()),
                                [key]: ev.target.value,
														...
														<div className="mt-3 flex gap-2 pt-2 border-t border-slate-100">
															<button type="button" disabled={savingId === e.id} onClick={() => void savePayment(e.id)} className="inline-flex items-center gap-1.5 bg-[#E61E32] hover:bg-[#c9182a] disabled:opacity-50 text-white text-xs font-semibold px-4 py-2 rounded-xl cursor-pointer transition-all shadow-xs">
																<CheckIcon className="size-3.5"/>
																{savingId === e.id ? 'Saving…' : 'Save Payment Details'}
															</button>
															<button type="button" disabled={savingId === e.id} onClick={() => void clearPayment(e.id)} className="text-xs text-slate-600 hover:text-slate-800 px-3 py-2 rounded-xl border border-slate-200 hover:bg-slate-50 cursor-pointer transition-all">
																Clear Details
															</button>
														</div>
													</div>
