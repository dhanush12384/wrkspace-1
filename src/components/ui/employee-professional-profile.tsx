...
import { EMPLOYMENT_TYPES, SKILL_CATEGORIES, newId, profileFromEmployee, type ProfessionalProfile, type ProfileAchievement, type ProfileCertification, type ProfileCustomSection, type ProfileEducation, type ProfileExperience, type ProfileInternship, type ProfileProject, type ProfilePublication, type SkillCategoryKey, } from '@/lib/employee-professional-profile';
type Props = {
    employee: any;
    onEmployeeUpdate?: (next: any) => void;
    compact?: boolean;
    saveOverride?: (profile: ProfessionalProfile) => Promise<{
        employee?: any;
...
async function fileToDataUrl(file: File, maxBytes = 350000): Promise<string> {
...
function AttachmentUpload({ sx, fileUrl, label, onUploaded, onCleared, onError, }: {
    sx: Sx;
    fileUrl?: string;
    label: string;
    ...
    return (<div className="flex flex-wrap items-center gap-2">
			...
			{has ? (<>
					...
					{onCleared ? (<button type="button" className="text-xs font-semibold text-[#B42318]" onClick={onCleared}>
							Remove file
						</button>) : null}
				</>) : null}
		</div>);
}
export function EmployeeProfessionalProfileEditor({ employee, onEmployeeUpdate, compact, saveOverride, canEditRemarks, }: Props) {
    ...
    const sx: Sx = {
        box: 'rounded-2xl border border-slate-100 bg-white p-5 space-y-4 shadow-[0_1px_4px_rgba(0,0,0,0.06)]',
        label: 'text-[10.5px] font-semibold uppercase tracking-widest text-slate-400',
        input: 'w-full rounded-xl border border-slate-300 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 outline-none focus:border-slate-900 focus:bg-white focus:ring-2 focus:ring-slate-900/10 transition-all',
        title: 'text-base font-semibold text-slate-900',
        muted: 'text-sm text-slate-500',
        btnGhost: 'inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-colors',
        btnDanger: 'rounded-lg p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors',
        tab: 'shrink-0 rounded-lg border border-slate-200 bg-white px-3.5 py-1.5 text-xs font-semibold text-slate-500 whitespace-nowrap hover:border-slate-900 hover:text-slate-900 transition-all',
        tabActive: 'shrink-0 rounded-lg border border-slate-900 bg-slate-900 px-3.5 py-1.5 text-xs font-semibold text-white whitespace-nowrap shadow-sm',
        chip: 'inline-flex items-center gap-1 rounded-full bg-slate-100 border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-700',
    };
    void compact;
    function setField<K extends keyof ProfessionalProfile>(key: K, value: ProfessionalProfile[K]) {
        setProfile((p) => ({ ...p, [key]: value }));
    }
    function addItem<K extends 'experience' | 'education' | 'projects' | 'certifications' | 'achievements' | 'internships' | 'publications' | 'customSections'>(key: K, factory: () => ProfessionalProfile[K][number]) {
        setProfile((p) => ({ ...p, [key]: [...(p[key] as any[]), factory()] }));
    }
    function updateItem<K extends 'experience' | 'education' | 'projects' | 'certifications' | 'achievements' | 'internships' | 'publications' | 'customSections'>(key: K, id: string, patch: Partial<ProfessionalProfile[K][number]>) {
        setProfile((p) => ({
            ...p,
            [key]: (p[key] as any[]).map((row) => (row.id === id ? { ...row, ...patch } : row)),
        }));
    }
    function removeItem<K extends 'experience' | 'education' | 'projects' | 'certifications' | 'achievements' | 'internships' | 'publications' | 'customSections'>(key: K, id: string) {
        ...
        try {
            ...
            if (employeeToken()) {
                const data = await apiPatch<{
                    employee?: any;
                    profile?: ProfessionalProfile;
                }>('/api/auth/me/profile', profile as any);
                onEmployeeUpdate?.({ ...employee, ...(data.employee || {}) });
                if (data.profile)
                    setProfile(data.profile);
            ...
            if (res.profile)
                setProfile(res.profile as ProfessionalProfile);
            setMsg({ type: 'ok', text: 'Profile saved — visible to verification companies.' });
        }
        catch (e: any) {
    ...
    if (!employee?.id) {
        return (<div className="rounded-xl border border-[#E2E8F0] bg-white p-6 text-sm text-[#64748B]">
				Loading your employee profile…
			</div>);
    }
    return (<div className="space-y-4">
			
			<div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-white px-5 py-4 shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
				<div>
					<h3 className="text-base font-semibold text-slate-900">Professional details</h3>
					<p className="mt-0.5 text-sm text-slate-500">
						Fill each section like a résumé — this is what verification companies see.
					</p>
				</div>
				<button type="button" disabled={saving} onClick={() => void save()} className="inline-flex items-center gap-1.5 rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-slate-800 disabled:opacity-60 transition-colors">
					<SaveIcon className="size-3.5"/>
					{saving ? 'Saving…' : 'Save profile'}
				</button>
			</div>

			{msg ? (<p className={msg.type === 'ok'
			...
			<div className="flex gap-1.5 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
				{TABS.map((t) => (<button key={t.key} type="button" onClick={() => setActiveTab(t.key)} className={activeTab === t.key ? sx.tabActive : sx.tab} title={t.hint}>
						{t.label}
					</button>))}
			</div>

			{activeTab === 'personal' && (<PersonalTab employee={employee} profile={profile} setField={setField} sx={sx} setMsg={setMsg}/>)}
			{activeTab === 'summary' && (<SummaryTab profile={profile} setField={setField} sx={sx} canEditRemarks={Boolean(canEditRemarks)} setMsg={setMsg}/>)}
			{activeTab === 'experience' && (<ExperienceTab profile={profile} addItem={addItem} updateItem={updateItem} removeItem={removeItem} sx={sx} setMsg={setMsg}/>)}
			{activeTab === 'education' && (<EducationTab profile={profile} addItem={addItem} updateItem={updateItem} removeItem={removeItem} sx={sx} setMsg={setMsg}/>)}
			{activeTab === 'skills' && (<SkillsTab profile={profile} addTag={addTag} removeTag={removeTag} sx={sx} setField={setField} setMsg={setMsg}/>)}
			{activeTab === 'projects' && (<ProjectsTab profile={profile} addItem={addItem} updateItem={updateItem} removeItem={removeItem} sx={sx} setMsg={setMsg}/>)}
			{activeTab === 'certifications' && (<CertificationsTab profile={profile} addItem={addItem} updateItem={updateItem} removeItem={removeItem} sx={sx} setMsg={setMsg}/>)}
			{activeTab === 'achievements' && (<AchievementsTab profile={profile} addItem={addItem} updateItem={updateItem} removeItem={removeItem} sx={sx} setMsg={setMsg}/>)}
			{activeTab === 'internships' && (<InternshipsTab profile={profile} addItem={addItem} updateItem={updateItem} removeItem={removeItem} sx={sx} setMsg={setMsg}/>)}
			{activeTab === 'publications' && (<PublicationsTab profile={profile} addItem={addItem} updateItem={updateItem} removeItem={removeItem} sx={sx} setMsg={setMsg}/>)}
			{activeTab === 'custom' && (<CustomSectionsTab profile={profile} addItem={addItem} updateItem={updateItem} removeItem={removeItem} sx={sx} setMsg={setMsg}/>)}

			<div className="flex justify-end">
				<button type="button" disabled={saving} onClick={() => void save()} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800 disabled:opacity-60 transition-colors">
...
function Field({ sx, label, value, onChange, placeholder, readOnly, required, }: {
    sx: Sx;
    label: string;
    value: string;
    ...
    return (<div className="space-y-1">
			...
			<input className={sx.input + (readOnly ? ' opacity-70 cursor-not-allowed' : '')} value={value} readOnly={readOnly} placeholder={placeholder} onChange={(e) => onChange?.(e.target.value)}/>
		</div>);
}
function TextArea({ sx, label, value, onChange, placeholder, rows = 3, }: {
    sx: Sx;
    label: string;
    value: string;
    onChange: (v: string) => void;
    ...
    return (<div className="space-y-1">
			...
			<textarea className={sx.input} rows={rows} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)}/>
		</div>);
}
function Select({ sx, label, value, onChange, options, }: {
    sx: Sx;
    label: string;
    value: string;
    onChange: (v: string) => void;
...
function EmptyHint({ sx, text }: {
    sx: Sx;
    text: string;
}) {
    return <p className={sx.muted}>{text}</p>;
}
function AddButton({ sx, onClick, label = 'Add' }: {
    sx: Sx;
    onClick: () => void;
    label?: string;
}) {
    return (<button type="button" className={sx.btnGhost} onClick={onClick}>
			<PlusIcon className="size-3.5"/> {label}
		</button>);
}
function RemoveButton({ sx, onClick, label }: {
    sx: Sx;
    onClick: () => void;
    label?: string;
}) {
    return (<button type="button" className={sx.btnDanger + ' inline-flex items-center gap-1'} onClick={onClick}>
...
function PersonalTab({ employee, profile, setField, sx, setMsg, }: {
    employee: any;
    profile: ProfessionalProfile;
    ...
    return (<div className="space-y-4">
			...
			<section className={sx.box}>
				...
				<AttachmentUpload sx={sx} fileUrl={profile.personalFileUrl} label="Upload personal document" onUploaded={(url) => setField('personalFileUrl', url)} onCleared={() => setField('personalFileUrl', '')} onError={(text) => setMsg({ type: 'err', text })}/>
			</section>
		</div>);
}
function SummaryTab({ profile, setField, sx, canEditRemarks, setMsg, }: {
    profile: ProfessionalProfile;
    setField: <K extends keyof ProfessionalProfile>(key: K, value: ProfessionalProfile[K]) => void;
    sx: Sx;
    canEditRemarks: boolean;
    ...
    return (<section className={sx.box}>
			...
			{canEditRemarks ? (<TextArea sx={sx} label="Admin remarks (admin only)" rows={3} value={profile.remarks} onChange={(v) => setField('remarks', v)} placeholder="Internal notes — only admins can set remarks and Active/Inactive status…"/>) : null}
			<div className="pt-2 space-y-2">
				...
				<AttachmentUpload sx={sx} fileUrl={profile.summaryFileUrl} label="Upload resume / summary file" onUploaded={(url) => setField('summaryFileUrl', url)} onCleared={() => setField('summaryFileUrl', '')} onError={(text) => setMsg({ type: 'err', text })}/>
			</div>
		</section>);
}
function ExperienceTab({ profile, addItem, updateItem, removeItem, sx, setMsg }: ListTabProps) {
    ...
    return (<section className={sx.box}>
			<div className="flex items-center justify-between">
				...
				<AddButton sx={sx} label="Add experience" onClick={() => addItem('experience', () => ({
            id: newId(),
            title: '',
            employmentType: '',
            company: '',
			...
			{list.map((x: ProfileExperience) => (<ItemCard key={x.id}>
					...
					<AttachmentUpload sx={sx} fileUrl={x.fileUrl} label="Upload experience letter / proof" onUploaded={(url) => updateItem('experience', x.id, { fileUrl: url })} onCleared={() => updateItem('experience', x.id, { fileUrl: '' })} onError={(text) => setMsg({ type: 'err', text })}/>
...
function EducationTab({ profile, addItem, updateItem, removeItem, sx, setMsg }: ListTabProps) {
    ...
    return (<section className={sx.box}>
			<div className="flex items-center justify-between">
				...
				<AddButton sx={sx} label="Add education" onClick={() => addItem('education', () => ({
            id: newId(),
            institution: '',
            degree: '',
            specialization: '',
			...
			{list.map((q: ProfileEducation) => (<ItemCard key={q.id}>
					<div className="grid gap-2 sm:grid-cols-2">
					...
					<AttachmentUpload sx={sx} fileUrl={q.fileUrl} label="Upload marksheet / degree" onUploaded={(url) => updateItem('education', q.id, { fileUrl: url })} onCleared={() => updateItem('education', q.id, { fileUrl: '' })} onError={(text) => setMsg({ type: 'err', text })}/>
					<RemoveButton sx={sx} label="Remove" onClick={() => removeItem('education', q.id)}/>
				</ItemCard>))}
		</section>);
}
function SkillsTab({ profile, addTag, removeTag, sx, setField, setMsg, }: {
    profile: ProfessionalProfile;
    ...
    return (<div className="space-y-4">
			<section className={sx.box}>
				...
				<AttachmentUpload sx={sx} fileUrl={profile.skillsFileUrl} label="Upload skills document" onUploaded={(url) => setField('skillsFileUrl', url)} onCleared={() => setField('skillsFileUrl', '')} onError={(text) => setMsg({ type: 'err', text })}/>
			</section>
            ...
            return (<section key={key} className={sx.box}>
						<p className={sx.label}>{label}</p>
						<div className="flex flex-wrap gap-2">
							{tags.length === 0 ? <EmptyHint sx={sx} text="None added yet."/> : null}
							{tags.map((t) => (<span key={t} className={sx.chip}>
									{t}
						...
						<div className="flex gap-2">
							<input className={sx.input} placeholder={`Add ${label.toLowerCase()} — press Enter`} value={draft} onChange={(e) => setDrafts((d) => ({ ...d, [key]: e.target.value }))} onKeyDown={(e) => {
...
function ProjectsTab({ profile, addItem, updateItem, removeItem, sx, setMsg }: ListTabProps) {
    ...
    return (<section className={sx.box}>
			<div className="flex items-center justify-between">
				...
				<AddButton sx={sx} label="Add project" onClick={() => addItem('projects', () => ({
            id: newId(),
            name: '',
            role: '',
            githubUrl: '',
			...
			{list.map((p: ProfileProject) => (<ItemCard key={p.id}>
					<div className="grid gap-2 sm:grid-cols-2">
					...
					<AttachmentUpload sx={sx} fileUrl={p.fileUrl} label="Upload screenshot / project file" onUploaded={(url) => updateItem('projects', p.id, { fileUrl: url })} onCleared={() => updateItem('projects', p.id, { fileUrl: '' })} onError={(text) => setMsg({ type: 'err', text })}/>
					<RemoveButton sx={sx} label="Remove" onClick={() => removeItem('projects', p.id)}/>
				</ItemCard>))}
		</section>);
}
function CertificationsTab({ profile, addItem, updateItem, removeItem, sx, setMsg, }: ListTabProps & {
    setMsg: (m: {
        type: 'ok' | 'err';
        text: string;
    } | null) => void;
    ...
    return (<section className={sx.box}>
			...
			{list.map((c: ProfileCertification) => (<ItemCard key={c.id}>
					<div className="grid gap-2 sm:grid-cols-2">
					...
					<AttachmentUpload sx={sx} fileUrl={c.fileUrl} label="Upload certificate" onUploaded={(url) => updateItem('certifications', c.id, { fileUrl: url })} onCleared={() => updateItem('certifications', c.id, { fileUrl: '' })} onError={(text) => setMsg({ type: 'err', text })}/>
					<RemoveButton sx={sx} label="Remove" onClick={() => removeItem('certifications', c.id)}/>
				</ItemCard>))}
		</section>);
}
function AchievementsTab({ profile, addItem, updateItem, removeItem, sx, setMsg, }: ListTabProps & {
    setMsg: (m: {
        type: 'ok' | 'err';
        text: string;
    } | null) => void;
    ...
    return (<section className={sx.box}>
			...
			{list.map((a: ProfileAchievement) => (<ItemCard key={a.id}>
					<div className="grid gap-2 sm:grid-cols-2">
					...
					<AttachmentUpload sx={sx} fileUrl={a.fileUrl} label="Upload proof / certificate" onUploaded={(url) => updateItem('achievements', a.id, { fileUrl: url })} onCleared={() => updateItem('achievements', a.id, { fileUrl: '' })} onError={(text) => setMsg({ type: 'err', text })}/>
...
function InternshipsTab({ profile, addItem, updateItem, removeItem, sx, setMsg }: ListTabProps) {
    ...
    return (<section className={sx.box}>
			<div className="flex items-center justify-between">
				...
				<AddButton sx={sx} label="Add internship" onClick={() => addItem('internships', () => ({
            id: newId(),
            title: '',
            company: '',
            location: '',
			...
			{list.map((x: ProfileInternship) => (<ItemCard key={x.id}>
					<div className="grid gap-2 sm:grid-cols-2">
					...
					<AttachmentUpload sx={sx} fileUrl={x.fileUrl} label="Upload internship letter / proof" onUploaded={(url) => updateItem('internships', x.id, { fileUrl: url })} onCleared={() => updateItem('internships', x.id, { fileUrl: '' })} onError={(text) => setMsg({ type: 'err', text })}/>
...
function PublicationsTab({ profile, addItem, updateItem, removeItem, sx, setMsg }: ListTabProps) {
    ...
    return (<section className={sx.box}>
			<div className="flex items-center justify-between">
				...
				<AddButton sx={sx} label="Add publication" onClick={() => addItem('publications', () => ({
            id: newId(),
            title: '',
            authors: '',
            journal: '',
			...
			{list.map((p: ProfilePublication) => (<ItemCard key={p.id}>
					...
					<AttachmentUpload sx={sx} fileUrl={p.fileUrl} label="Upload paper / PDF" onUploaded={(url) => updateItem('publications', p.id, { fileUrl: url })} onCleared={() => updateItem('publications', p.id, { fileUrl: '' })} onError={(text) => setMsg({ type: 'err', text })}/>
...
function CustomSectionsTab({ profile, addItem, updateItem, removeItem, sx, setMsg }: ListTabProps) {
    ...
    return (<section className={sx.box}>
			<div className="flex items-center justify-between">
				...
				<AddButton sx={sx} label="Add section" onClick={() => addItem('customSections', () => ({ id: newId(), title: '', content: '', fileUrl: '' }))}/>
			</div>
			{list.length === 0 ? <EmptyHint sx={sx} text="No custom sections yet."/> : null}
			{list.map((c: ProfileCustomSection) => (<ItemCard key={c.id}>
					...
					<AttachmentUpload sx={sx} fileUrl={c.fileUrl} label="Upload supporting file" onUploaded={(url) => updateItem('customSections', c.id, { fileUrl: url })} onCleared={() => updateItem('customSections', c.id, { fileUrl: '' })} onError={(text) => setMsg({ type: 'err', text })}/>
...
type ListTabProps = {
    profile: ProfessionalProfile;
    addItem: <K extends 'experience' | 'education' | 'projects' | 'certifications' | 'achievements' | 'internships' | 'publications' | 'customSections'>(key: K, factory: () => ProfessionalProfile[K][number]) => void;
    updateItem: <K extends 'experience' | 'education' | 'projects' | 'certifications' | 'achievements' | 'internships' | 'publications' | 'customSections'>(key: K, id: string, patch: Partial<ProfessionalProfile[K][number]>) => void;
    removeItem: <K extends 'experience' | 'education' | 'projects' | 'certifications' | 'achievements' | 'internships' | 'publications' | 'customSections'>(key: K, id: string) => void;
    sx: Sx;
    setMsg: (m: {
        type: 'ok' | 'err';
        text: string;
