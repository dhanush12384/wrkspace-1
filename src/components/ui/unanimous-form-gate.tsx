...
import { ShieldAlert, MessageSquare, AlertCircle, Calendar, UserCheck, CheckSquare, ArrowRight, Loader2, Lock } from 'lucide-react';
...
export function UnanimousFormGate({ userEmail, userName, userType, userId, onSuccess, }: UnanimousFormGateProps) {
    const [checking, setChecking] = useState(true);
    const [submitting, setSubmitting] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const isDark = userType === 'ADMIN';
    const [comfortableSharing, setComfortableSharing] = useState<string | null>(null);
    ...
    if (checking) {
        return (<div className={cn("fixed inset-0 flex flex-col items-center justify-center z-50 transition-colors duration-300", isDark ? "bg-zinc-950 text-zinc-200" : "bg-[#e8edf5] text-slate-800")}>
				<div className="relative">
					<div className={cn("absolute inset-0 blur-xl rounded-full scale-150 animate-pulse", isDark ? "bg-brand-500/20" : "bg-brand-500/10")}/>
					<div className="relative flex flex-col items-center gap-4">
						<Loader2 className="size-10 text-brand-500 animate-spin"/>
						<p className={cn("text-xs tracking-widest uppercase", isDark ? "text-zinc-400" : "text-slate-500")}>Loading Portal...</p>
					</div>
				</div>
			</div>);
    }
    return (<div className={cn("fixed inset-0 overflow-y-auto z-50 flex justify-center p-4 md:p-8 selection:bg-brand-500/35 selection:text-white transition-colors duration-300", isDark ? "bg-zinc-950 text-zinc-200" : "bg-[#e8edf5] text-slate-800")}>
			
			<div className={cn("fixed inset-0 pointer-events-none transition-opacity duration-300", isDark
			...
			<div className="relative w-full max-w-3xl my-auto space-y-8 py-10">
				...
				<div className="text-center space-y-4 max-w-xl mx-auto">
					<div className={cn("inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] tracking-widest uppercase shadow-sm transition-all duration-300", isDark
            ...
            : "bg-white text-slate-500")}>
					...
					<h1 className={cn("text-3xl md:text-4xl tracking-tight transition-colors duration-300", isDark ? "text-white" : "text-slate-900")}>
						Feedback Portal
					</h1>
					<p className={cn("text-sm transition-colors duration-300", isDark ? "text-zinc-400" : "text-slate-600")}>
						This dashboard values transparency and safety. Please share your feedback before accessing your main control center.
					</p>
				</div>

				<form onSubmit={handleSubmit} className={cn("rounded-2xl p-6 md:p-10 backdrop-blur-xl shadow-2xl relative overflow-hidden transition-all duration-300", isDark
            ...
            : "bg-white border-transparent text-slate-800 shadow-xl shadow-slate-200/50")}>
					...
					{error && (<div className={cn("mb-6 p-4 rounded-xl border text-xs flex items-center gap-3", isDark
                ...
                : "border-transparent bg-red-50 text-red-605")}>
					...
					<div className="space-y-8">
						...
						<div className="space-y-4">
							<label className={cn("block text-sm md:text-base transition-colors duration-300", isDark ? "text-zinc-100" : "text-slate-900")}>
								Is there something you have wanted to tell us but haven&apos;t felt comfortable sharing directly?
							</label>
							<div className="grid grid-cols-2 gap-4">
								<button type="button" onClick={() => setComfortableSharing('Yes')} className={cn('relative py-4 px-6 rounded-xl border text-sm transition-all duration-300 text-center flex flex-col items-center justify-center gap-2 cursor-pointer', comfortableSharing === 'Yes'
            ? isDark
            ...
            : isDark
                ...
                : 'bg-slate-50/50 border-transparent text-slate-550 hover:bg-slate-100/80 hover:text-slate-800')}>
								...
								<button type="button" onClick={() => setComfortableSharing('No')} className={cn('relative py-4 px-6 rounded-xl border text-sm transition-all duration-300 text-center flex flex-col items-center justify-center gap-2 cursor-pointer', comfortableSharing === 'No'
            ? isDark
            ...
            : isDark
                ...
                : 'bg-slate-50/50 border-transparent text-slate-550 hover:bg-slate-100/80 hover:text-slate-800')}>
						...
						<AnimatePresence initial={false}>
							{comfortableSharing === 'Yes' && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.4, ease: 'easeInOut' }} className={cn("overflow-hidden space-y-8 pt-6 border-t", isDark ? "border-zinc-800/50" : "border-slate-100")}>
									
									<div className="space-y-2.5">
										<label className={cn("flex items-center gap-2 text-sm", isDark ? "text-zinc-200" : "text-slate-800")}>
											<MessageSquare className="size-4 text-brand-500"/>
											<span>What would you like to tell us? *</span>
										</label>
										<p className={cn("text-[11px] italic", isDark ? "text-zinc-500" : "text-slate-450")}>
											Example prompt: “Share anything you feel you cannot openly discuss with your manager or team.”
										</p>
										<textarea rows={5} required value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} placeholder="Write your concerns or feedback in detail here..." className={cn("w-full border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-brand-500 focus:border-brand-500 transition-colors", isDark
                ...
                : "bg-slate-50 border-transparent text-slate-900 placeholder:text-slate-450 focus:bg-white")}/>
									...
									<div className="space-y-3">
										...
										<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
											{concernOptions.map((option) => (<button key={option} type="button" onClick={() => toggleConcern(option)} className={cn('flex items-center gap-3 px-4 py-2.5 rounded-xl border text-left text-xs transition-all duration-200 cursor-pointer', selectedConcerns.includes(option)
                    ? isDark
                    ...
                    : isDark
                        ...
                        : 'bg-slate-50/40 border-transparent text-slate-550 hover:bg-slate-100 hover:text-slate-700')}>
													<div className={cn('size-3.5 rounded border flex items-center justify-center text-[8px]', selectedConcerns.includes(option)
                        ...
                        : 'border-slate-200 bg-white')}>
									...
									<div className="space-y-2.5">
										...
										<div className="grid grid-cols-4 gap-2">
											{severityOptions.map((opt) => (<button key={opt} type="button" onClick={() => setSeverity(opt)} className={cn('py-2.5 rounded-xl border text-xs transition-all duration-250 cursor-pointer', severity === opt
                    ? opt === 'Urgent'
                        ...
                        : 'bg-slate-50/40 border-transparent text-slate-500 hover:bg-slate-100 hover:text-slate-700')}>
									...
									<div className="space-y-2.5">
										...
										<div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
											{durationOptions.map((opt) => (<button key={opt} type="button" onClick={() => setDuration(opt)} className={cn('py-2 px-1.5 rounded-xl border text-[11px] text-center transition-all duration-200 cursor-pointer', duration === opt
                    ? isDark
                        ...
                        : 'bg-slate-50/40 border-transparent text-slate-500 hover:bg-slate-100 hover:text-slate-700')}>
									...
									<div className="space-y-2.5">
										...
										<div className="grid grid-cols-3 gap-2">
											{involvesOthersOptions.map((opt) => (<button key={opt} type="button" onClick={() => setInvolvesOthers(opt)} className={cn('py-2.5 rounded-xl border text-xs transition-all duration-200 cursor-pointer', involvesOthers === opt
                    ? isDark
                    ...
                    : isDark
                        ...
                        : 'bg-slate-50/40 border-transparent text-slate-550 hover:bg-slate-100 hover:text-slate-700')}>
									...
									<div className="space-y-3">
										...
										<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
											{actionOptions.map((option) => (<button key={option} type="button" onClick={() => toggleAction(option)} className={cn('flex items-center gap-3 px-4 py-2.5 rounded-xl border text-left text-xs transition-all duration-200 cursor-pointer', selectedActions.includes(option)
                    ? isDark
                        ? 'bg-brand-500/10 border-brand-550 text-white'
                        : 'bg-brand-50 border-transparent text-brand-700'
                    : isDark
                        ? 'bg-zinc-950/40 border-zinc-800 text-zinc-400 hover:border-zinc-750 hover:text-zinc-350'
                        : 'bg-slate-50/40 border-transparent text-slate-550 hover:bg-slate-100 hover:text-slate-700')}>
													<div className={cn('size-3.5 rounded border flex items-center justify-center text-[8px]', selectedActions.includes(option)
                    ? 'border-brand-500 bg-brand-500 text-white'
                    : isDark
                        ? 'border-zinc-700 bg-zinc-900/60'
                        : 'border-slate-200 bg-white')}>
									...
									<div className="space-y-2.5">
										...
										<textarea rows={3} value={additionalNotes} onChange={(e) => setAdditionalNotes(e.target.value)} placeholder="Optional additional context, details, or requests..." className={cn("w-full border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-brand-500 focus:border-brand-500 transition-colors", isDark
                ? "bg-zinc-950 border-zinc-800 text-zinc-100 placeholder:text-zinc-650"
                : "bg-slate-50 border-transparent text-slate-900 placeholder:text-slate-450 focus:bg-white")}/>
									</div>
								</motion.div>)}
					...
					{comfortableSharing !== null && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className={cn("mt-8 pt-6 border-t flex items-center justify-between", isDark ? "border-zinc-850/60 text-zinc-550" : "border-slate-200 text-slate-500")}>
							<div />

							<Button type="submit" disabled={submitting} className="bg-brand-600 hover:bg-brand-500 text-white text-xs px-6 py-2.5 h-auto transition-all shadow-md shadow-brand-600/10 active:scale-[0.98] cursor-pointer inline-flex items-center gap-2 animate-none">
								{submitting ? (<>
