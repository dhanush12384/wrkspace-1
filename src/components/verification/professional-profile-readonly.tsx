...
export function ProfessionalProfileReadonly({ profile, employee, showRemarks = false }: Props) {
    ...
    return (<>
			<div className="ev-card" style={{ marginBottom: 12 }}>
				...
				{p.personalFileUrl ? (<p style={{ marginTop: 10 }}>
			...
			<div className="ev-card" style={{ marginBottom: 12 }}>
				...
				{p.summaryFileUrl ? (<p style={{ marginTop: 10 }}>
						<a href={p.summaryFileUrl} target="_blank" rel="noreferrer">
							View resume / summary file
						</a>
					</p>) : null}
				{showRemarks ? (<div className="ev-subsection">
						<p className="ev-muted" style={{ marginBottom: 4 }}>
							Remarks
						</p>
			...
			<div className="ev-card" style={{ marginBottom: 12 }}>
				...
				{education.length === 0 ? (<p className="ev-muted">None listed.</p>) : (<ul className="ev-notes">
						{education.map((q: any) => (<li key={q.id || q.degree}>
								...
								{q.from || q.to ? (<span className="ev-muted">
										{' '}
										· {q.from || '?'} – {q.to || '?'}
									</span>) : null}
			...
			<div className="ev-card" style={{ marginBottom: 12 }}>
				<h3>Skills</h3>
				{p.skillsFileUrl ? (<p style={{ marginBottom: 10 }}>
			...
			<div className="ev-card" style={{ marginBottom: 12 }}>
				...
				{publications.length === 0 ? (<p className="ev-muted">None listed.</p>) : (<ul className="ev-notes">
						{publications.map((pub: any) => (<li key={pub.id || pub.title}>
								...
								{pub.journal || pub.conference ? (<span className="ev-muted"> · {pub.journal || pub.conference}</span>) : null}
			...
			{customSections.length > 0 ? (<div className="ev-card" style={{ marginBottom: 12 }}>
					<h3>Custom sections</h3>
					{customSections.map((c: any) => (<div key={c.id || c.title} className="ev-subsection">
							<p className="ev-muted" style={{ marginBottom: 4 }}>
								{c.title || 'Untitled'}
							</p>
