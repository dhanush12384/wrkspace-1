...
export function PeerColleagueView({ authHeaders }: Props) {
	...
	// Load organizational directory on mount
	useEffect(() => {
		let active = true;
	...
	// Filter colleagues list based on search query
	...
	// Trigger OTP request when a colleague card is selected
	...
	// 6-digit OTP fields change handling
					...
					<div className="flex flex-wrap items-center justify-between gap-4">
						<div className="flex items-center gap-4">
							{photo ? (
								...
								<div className="size-16 rounded-xl bg-slate-100 flex items-center justify-center font-bold text-slate-800 text-lg border border-slate-200">
									{String(e.name || '?')
										.split(/\s+/)
										.filter(Boolean)
										.slice(0, 2)
						...
						<button
							type="button"
							className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"
							onClick={reset}
						>
	...
	if (step === 'otp' && selectedColleague) {
		...
		// Extract masked email from the success message
		...
		return (
			<div className="ev-shell flex flex-col items-center justify-center py-10">
				<div className="w-full max-w-[420px] rounded-2xl border border-slate-100 bg-white p-6 shadow-[0_4px_16px_rgba(0,0,0,0.03)]">
					<button
						type="button"
						onClick={reset}
						className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400 hover:text-slate-900 mb-6 transition-colors cursor-pointer"
					>
						<ArrowLeft className="size-3.5" />
						Back to directory
					</button>
					...
					<div className="flex items-center gap-3.5 pb-4 border-b border-slate-100">
						{photo ? (
							<img
								src={photo}
								alt=""
								className="size-11 rounded-lg object-cover border border-slate-200"
							/>
						) : (
							<div className="size-11 rounded-lg bg-slate-100 flex items-center justify-center font-bold text-slate-800 border border-slate-200 text-sm">
					...
					{err && (
						<div className="mt-4 rounded-lg bg-red-50 border border-red-100 p-2.5 text-center text-xs font-medium text-red-600">
							{err}
						</div>
					)}
					...
					<form onSubmit={verifyOtp} className="mt-6">
						<div className="flex justify-between items-center gap-2" onPaste={handleOtpPaste}>
							{otpDigits.map((digit, index) => (
								<input
									...
									className="size-10 rounded-lg border border-slate-300 bg-slate-50 text-center text-lg font-bold text-slate-900 outline-none focus:border-slate-900 focus:bg-white focus:ring-2 focus:ring-slate-900/10 transition-all"
									disabled={busy}
								/>
							))}
						</div>
						...
						<button
							...
							className="mt-6 flex h-10 w-full items-center justify-center rounded-xl bg-slate-900 text-xs font-semibold text-white transition-all hover:bg-slate-800 disabled:opacity-50 cursor-pointer shadow-sm"
						>
						...
						<button
							...
							onClick={resendOtp}
							disabled={busy}
							className="mt-3.5 w-full text-center text-xs font-semibold text-slate-500 hover:text-slate-900 transition-colors cursor-pointer"
						>
							Resend OTP
						</button>
					</form>
	...
	return (
		<div className="ev-shell">
			...
			<div className="flex flex-wrap items-center justify-between gap-4 mb-6">
				...
				<div className="relative w-full sm:max-w-xs">
					...
					<input
						...
						className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-300 bg-white text-xs text-slate-800 placeholder:text-slate-400 outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-900/10 transition-all shadow-[0_1px_2px_rgba(0,0,0,0.02)]"
					/>
				</div>
			</div>
			...
			{loading ? (
				...
				<div className="rounded-2xl border border-slate-100 bg-white p-12 text-center shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
					{filteredColleagues.map((emp) => {
						...
						return (
							<div
								key={emp.id}
								onClick={() => !busy && selectColleagueAndSendOtp(emp)}
								className="group mx-auto w-full max-w-[290px] rounded-[32px] border border-slate-200 bg-white p-4 flex flex-col gap-4.5 transition-all hover:border-slate-900/60 hover:shadow-lg cursor-pointer"
							>
								...
								{/* Bottom Footer: Handle/ID and button */}
								<div className="flex items-center justify-between pt-1 border-t border-slate-50">
									<div className="flex items-center gap-2">
										{photo ? (
											...
											<div className="size-7 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-700 text-[10px] border border-slate-200">
												{String(emp.name || '?')
													.split(/\s+/)
													.filter(Boolean)
													.slice(0, 2)
									...
									<button
										type="button"
										className="inline-flex items-center gap-1 rounded-xl bg-slate-950 px-3.5 py-2 text-[10px] font-bold text-white shadow-sm hover:bg-slate-800 transition-colors"
									>
										+ Verify
									</button>
								</div>
