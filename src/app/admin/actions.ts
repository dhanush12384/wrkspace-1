...
export async function loginAdminWithGoogle(email: string) {
    try {
        ...
        if (!normalized)
            return { success: false as const, error: 'Google account email missing' };
        const admin = await db.admin.findUnique({
...
export async function getEmployees() {
    try {
        ...
        return employees.map((e) => {
            const { photoUrl, ...rest } = e as typeof e & {
...
export async function updateEmployeePhoto(employeeId: string, photoUrl: string | null) {
    try {
        ...
        if (photoUrl != null && String(photoUrl).trim()) {
            ...
            if (s.startsWith('data:image/') && s.includes(';base64,')) {
                if (s.length > 450000)
                    return { success: false as const, error: 'Photo too large — try a smaller image' };
                next = s;
            }
            else if (/^https?:\/\//.test(s)) {
                next = s;
...
export async function updateEmployeeProfessionalProfile(employeeId: string, payload: Record<string, unknown>) {
    try {
        const id = String(employeeId || '').trim();
        if (!id)
            return { success: false as const, error: 'Employee id required' };
        const { sanitizeProfessionalProfile, profileFromEmployee } = await import('@/lib/employee-professional-profile');
...
export async function setEmployeeHomeLocation(employeeId: string, data: {
    lat: number;
    lng: number;
...
export async function getFcmPushStatus() {
    ...
    try {
        ...
        const firebaseConfigured = Boolean(process.env.FIREBASE_SERVICE_ACCOUNT_JSON && String(process.env.FIREBASE_SERVICE_ACCOUNT_JSON).trim());
        const vapidConfigured = Boolean(process.env.NEXT_PUBLIC_FIREBASE_VAPID_KEY && String(process.env.NEXT_PUBLIC_FIREBASE_VAPID_KEY).trim());
        return {
            tokens,
            firebaseConfigured,
            vapidConfigured,
...
async function sendTaskEmail(toEmail: string, taskTitle: string, taskDesc: string, deadline: Date, allocatorName: string, allocatorRole: string) {
    try {
        ...
        if (data.assigneeId === 'ALL') {
            ...
            db.employee.findMany().then(employees => {
                employees.forEach(emp => {
                    if (emp.email) {
                        sendTaskEmail(emp.email, data.title, data.description, new Date(data.deadline), data.allocatorName || 'Admin', data.allocatorRole || 'Admin').catch(err => console.error('Failed to send task email to ' + emp.email, err));
                    }
                });
...
export async function keepCheckedIn(employeeId: string, reason = 'office_work') {
    try {
        ...
        try {
            const { emitAttendanceUpdate } = await import('@/lib/realtime-emit');
            void emitAttendanceUpdate(employeeId, activeLog, reason === 'office_work' ? 'office-work' : 'keep-checked-in');
        }
        catch (_) { }
...
export async function toggleMessageReaction(messageId: string, userId: string, userName: string, emoji: string) {
    try {
...
export async function postMessageWithAttachment(channel: string, senderId: string, senderName: string, content: string, senderRole: string, attachment?: {
    ...
    try {
        ...
        if (attachmentUrl && attachmentUrl.length > 1800000) {
...
export async function editEmployeeWorkSubmission(submissionId: string, employeeId: string, updates: {
    title: string;
    description: string;
    hoursSpent: number;
}) {
...
function generateRealisticLeads(city: string, category: string, count: number, sources: string[]): any[] {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (leads.some(l => l.businessName === businessName))
            continue;
...
export async function updateHrCompany(id: string, data: {
    companyName?: string;
    website?: string;
    industry?: string;
    location?: string;
...
export async function triggerHrCompaniesCrawl(city: string): Promise<{
    success: boolean;
    count?: number;
    error?: string;
    companies?: any[];
...
export async function giveBadgeToEmployee(employeeId: string, badge: {
    title: string;
    icon: string;
    color: string;
    description?: string;
...
export async function sendBulkAlerts(subject: string, bodyText: string, employeeIds?: string[]) {
    try {
        ...
        await addAlertJobsToQueue(employees.map(emp => emp.email), subjectClean, bodyClean);
...
export async function checkHasSubmittedFeedback(email: string) {
    try {
        if (!email)
            return { success: false, hasSubmitted: false, error: 'Email is required' };
