...
const MAX_RELOADS = 2;
function isChunkFailure(error: Error & {
    digest?: string;
}) {
...
export default function Error({ error, reset, }: {
    ...
    return (<main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[#0B1220] px-6 text-center text-white">
			...
			{detail ? (<p className="max-w-sm break-words rounded-md bg-white/5 px-3 py-2 text-left font-mono text-[11px] text-amber-200/90">
					{detail}
				</p>) : null}
			<div className="flex gap-3 pt-2">
				<button type="button" onClick={() => window.location.reload()} className="rounded-md bg-white px-4 py-2 text-sm font-semibold text-black">
					Reload
				</button>
				<button type="button" onClick={() => reset()} className="rounded-md border border-white/40 px-4 py-2 text-sm font-semibold text-white">
					Try again
				</button>
			</div>
		</main>);
