...
export default function EventTicketPage() {
    ...
    if (loading) {
        return (<div className="employee-portal min-h-screen bg-[#e8edf5] flex items-center justify-center text-slate-700 font-mono text-sm font-semibold">
				Loading event pass…
			</div>);
    }
    if (!event) {
        return (<div className="employee-portal min-h-screen bg-[#e8edf5] flex flex-col items-center justify-center gap-4 text-center p-6">
    ...
    return (<div className="employee-portal min-h-screen bg-[#e8edf5] text-slate-900 font-sans selection:bg-brand-200 p-4 md:p-8 flex flex-col items-center justify-center">
            ...
            if (window.opener)
				...
				<img src="/branding/wrkspace-logo.png?v=20260717b" alt="wrkspace" className="emp-logo-mark"/>
			</div>
			...
			<div className="w-full max-w-2xl bg-white border border-slate-300 shadow-lg relative flex flex-col md:flex-row overflow-hidden">
				...
				<div className="w-full md:w-2/5 h-64 md:h-auto bg-slate-100 flex items-center justify-center border-b md:border-b-0 md:border-r border-slate-300 relative">
					{event.imageUrl ? (<img src={event.imageUrl} alt={event.title} className="w-full h-full object-contain"/>) : (<div className="absolute inset-0 bg-gradient-to-br from-brand-100 via-slate-50 to-white flex items-center justify-center">
				...
				<div className="w-full md:w-3/5 p-6 flex flex-col justify-between space-y-6">
					<div className="space-y-4">
						{employee && (<div className="border border-dashed border-slate-400 bg-slate-50 p-4 font-mono space-y-2.5 text-center relative overflow-hidden">
								{event.imageUrl && (<div className="w-full h-16 overflow-hidden relative border border-slate-300 mb-2 shrink-0 bg-white">
								...
								{employee.photoUrl ? (<img src={employee.photoUrl} alt="" className="mx-auto size-16 rounded-full object-cover border-2 border-brand-300 bg-white"/>) : (<div className="mx-auto size-16 rounded-full bg-brand-100 border-2 border-brand-300 flex items-center justify-center text-brand-800 font-black text-lg">
						...
						{reps.length > 0 && (<div className="space-y-1.5 pt-3 border-t border-slate-200">
								<span className="text-[11px] text-slate-700 font-bold uppercase tracking-wider font-mono block">
									Assigned Representatives
								</span>
								<div className="flex flex-wrap gap-1.5">
									{reps.map((r, i) => (<span key={i} className={`text-[11px] px-2 py-0.5 border font-semibold ${employee && r.name === `${employee.firstName} ${employee.lastName}`
                    ? 'bg-brand-50 border-brand-300 text-brand-900'
                    : 'bg-slate-50 border-slate-300 text-slate-800'}`}>
											{r.name}
											{employee && r.name === `${employee.firstName} ${employee.lastName}` && (<span className="ml-1 text-brand-600 font-bold">· You</span>)}
										</span>))}
								</div>
							</div>)}
					</div>
					...
					<div className="flex items-center gap-3 pt-3 border-t border-slate-200">
						<button onClick={() => window.print()} className="flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-500 text-white text-sm font-bold px-4 py-2.5 rounded-none cursor-pointer transition-colors w-full">
