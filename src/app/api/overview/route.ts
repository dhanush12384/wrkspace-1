...
export async function GET(req: NextRequest) {
    try {
        ...
        const [tasks, leavesPending, submissions, leadsActive, allEvents, todayAttendance] = await Promise.all([
            db.task.findMany({
                where: {
        ...
        const eventsCount = allEvents.filter((e) => eventHasRepresentative(e.representatives, uid)).length;
