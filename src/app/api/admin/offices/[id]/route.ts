...
export async function PATCH(req: NextRequest, ctx: {
    ...
    try {
        ...
        if (body.lat != null)
            data.lat = Number(body.lat);
        if (body.lng != null)
            data.lng = Number(body.lng);
        if (body.plusCode !== undefined)
            data.plusCode = body.plusCode ? String(body.plusCode) : null;
        if (body.radiusMeters != null)
            data.radiusMeters = Number(body.radiusMeters);
        if (body.geofenceM != null)
            data.geofenceM = Number(body.geofenceM);
        if (body.active != null)
