...
export async function PATCH(req: NextRequest) {
	try {
		...
		const shiftCheckIn =
			checkInRaw === null || checkInRaw === '' ? null : normalizeTimeLabel(checkInRaw);
		const shiftCheckOut =
			checkOutRaw === null || checkOutRaw === '' ? null : normalizeTimeLabel(checkOutRaw);
		if (checkInRaw && shiftCheckIn == null) return jsonError('Invalid shiftCheckIn');
		if (checkOutRaw && shiftCheckOut == null) return jsonError('Invalid shiftCheckOut');
