...
import { assertCanCheckIn } from '@/lib/shift-jobs';
...
export async function POST(req: NextRequest) {
    try {
        ...
        try {
            ...
            return Response.json(e.payload || { error: e.message, code: e.code }, { status: e.status || 403 });
        }
        ...
        if (!match)
            return jsonError('Invalid or unknown QR code');
