...
function requireMember(req: NextRequest): {
    viewerId: string;
    viewerEmail: string;
} {
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!employeeId)
            return jsonError('Employee ID required', 400);
        const target = await findEmployeeById(employeeId);
        if (!target)
            return jsonError('No employee found with that ID', 404);
        ...
        try {
            ...
            return jsonError(`Could not send OTP email to the employee (${maskEmail(target.email)}): ${mailErr?.message || 'SMTP error'}. Try again later.`, 502);
        }
        try {
            await resend.emails.send({
                from: 'Employee Verification Portal <support@app.redlix.co.in>',
